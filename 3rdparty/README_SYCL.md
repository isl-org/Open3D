...
4. Check your environment
   ...
   which python     # ${VIRTUAL_ENV}/bin/python
   python --version
   ```
5. Config and build
   ```bash
   ...
   make -j$(nproc)
   ```
6. Inspect devices and run SYCL tests
  We provide C++ and Python checks for device discovery and capabilities.
   ```bash
   # C++ test
   make tests -j$(nproc)
  ./bin/tests --gtest_filter="*Print*SYCLDevices"

   # Python
   make install-pip-package -j$(nproc)
  pytest ../python/test/core/test_sycl_utils.py -s -k "print_sycl_devices"
   ```
...
- `core::Device("SYCL:0")` maps the default SYCL GPU returned by
  `sycl::gpu_selector()` and `core::Device("SYCL:1") maps the fallback SYCL CPU device returned by `stcl::cpu_selector()`.
  ...
  - Currently we only support one SYCL GPU. Multi-GPU support will be added in
    the future.
  - The SYCL CPU device is used for testing in CI -- it does not support functionality such as linear algebra (some tensor operations), ray tracing and sparse / continuous convolutions.
- If SYCL GPU device is not available (e.g. when you don't have an Intel
  ...
  is not properly installed), `SYCL:0` will instead be the SYCL host device and there will be no `SYCL:1` device.
