    ...
    return queue.submit([&](sycl::handler& cgh) {
        ...
        cgh.parallel_for(
                ...
                [=](sycl::nd_item<1> item) [[intel::kernel_args_restrict]] {
                    ...
                    TFeat* out_column =
                            columns + size_t(filter_size_x) * filter_size_y *
                                              filter_size_z * in_channels *
                                              size_t(col_idx);
                    ...
                    const TReal out_pos[3] = {out_positions[out_idx * 3 + 0],
                    ...
                    for (int64_t n_idx = neighbor_start; n_idx < neighbor_end;
                        ...
                        ComputeFilterCoordinatesSYCL<ALIGN_CORNERS, MAPPING>(
                                x, y, z, filter_size_x, filter_size_y,
                                filter_size_z, inv_extents[0], inv_extents[1],
                                inv_extents[2], offset[0], offset[1],
                                offset[2]);
                        ...
                        for (int ic = static_cast<int>(lid); ic < in_channels;
                            ...
                            for (int j = 0; j < NUM_INTERP_VALUES; ++j) {
                                ...
                                out_column[interp_indices[j] * in_channels +
                                           ic] += value;
                            }
                        }
                    }  // for n
                    ...
                    for (int64_t n_idx = neighbor_start; n_idx < neighbor_end;
                        ...
                        if (individual_extent) {
                            if (isotropic_extent) {
                                ...
                                inv_extents[0] =
                                        TReal(1) / extents[3 * inp_idx + 0];
                                inv_extents[1] =
                                        TReal(1) / extents[3 * inp_idx + 1];
                                inv_extents[2] =
                                        TReal(1) / extents[3 * inp_idx + 2];
                            }
                        }
