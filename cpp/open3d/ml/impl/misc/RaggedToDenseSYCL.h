...
// default_value when the row is shorter than out_col_size. Single-stage op:
// launched via core::ParallelFor(queue, n, func).
...
void RaggedToDenseSYCL(sycl::queue& queue,
    ...
    core::ParallelFor(
            queue, static_cast<int64_t>(num_rows), [=](int64_t idx64) {
                const size_t i = static_cast<size_t>(idx64);
