...
at::Tensor ball_query(at::Tensor xyz, at::Tensor center, 
    double radius, const int64_t nsample) {    
    ...
    torch::Tensor out = at::zeros(
        {batch_size, ball_num, nsample}, at::dtype(ToTorchDtype<int>()).device(device)); 
    ...
    ball_query_launcher(batch_size, pts_num, ball_num, radius, nsample, center_data, xyz_data, idx, stream);
    return out;
}
