...
std::tuple<at::Tensor, at::Tensor> three_nn(at::Tensor query_pts, at::Tensor data_pts) {
    int batch_size = query_pts.size(0);
    int pts_num_out = query_pts.size(1);
    int pts_num_in = data_pts.size(1);

    auto device = data_pts.device();
    torch::Tensor out_idx = at::zeros(
        {batch_size, pts_num_out, 3}, at::dtype(ToTorchDtype<int>()).device(device)); 

    torch::Tensor out_dist2 = at::zeros(
        {batch_size, pts_num_out, 3}, at::dtype(ToTorchDtype<float>()).device(device)); 
    ...
    three_nn_launcher(batch_size, pts_num_out, pts_num_in, pts_out, pts_in, dist2, idx, stream);

    return std::tuple<at::Tensor, at::Tensor>(out_dist2, out_idx);
}


at::Tensor three_interpolate(at::Tensor points, at::Tensor idx, at::Tensor weights) {
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
    three_interpolate_launcher(batch_size, C, M, N, points_data, idx_data, weights_data, out_data, stream);

    return out;
}

at::Tensor three_interpolate_grad(at::Tensor grad_out, at::Tensor idx, at::Tensor weights, const int64_t M) {
    ...
    torch::Tensor out = at::zeros(
        {batch_size, C, M}, at::dtype(ToTorchDtype<float>()).device(device)); 
    ...
    cudaStream_t stream = at::cuda::getCurrentCUDAStream().stream();
    three_interpolate_grad_launcher(batch_size, C, N, M, grad_out_data, idx_data, weights_data, out_data, stream);
