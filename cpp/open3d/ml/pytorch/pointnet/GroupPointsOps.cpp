...
at::Tensor group_points_grad(at::Tensor grad_out, at::Tensor idx, const int64_t N) {
    ...
    group_points_grad_launcher(batch_size, C, N, feature_size, sample_size, grad_out_data, idx_data, grad_points, stream);
    return out;
}
...
at::Tensor group_points(at::Tensor points, at::Tensor idx) {
    ...
    torch::Tensor out = at::zeros(
        {batch_size, C, feature_size, sample_size}, at::dtype(ToTorchDtype<float>()).device(device)); 
    ...
    group_points_launcher(batch_size, C, N, feature_size, sample_size, points_data, idx_data, out_data, stream);
    return out;
}
