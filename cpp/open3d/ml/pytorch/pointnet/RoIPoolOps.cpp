    ...
    for (i = 0; i < boxes_num; i++) {
        for (j = 0; j < pts_num; j++) {
            cur_in_flag = pt_in_box3d_cpu(
                    pts_flat[j * 3], pts_flat[j * 3 + 1], pts_flat[j * 3 + 2],
                    boxes3d_flat[i * 7], boxes3d_flat[i * 7 + 1],
                    boxes3d_flat[i * 7 + 2], boxes3d_flat[i * 7 + 3],
                    boxes3d_flat[i * 7 + 4], boxes3d_flat[i * 7 + 5],
                    boxes3d_flat[i * 7 + 6]);
    ...
    for (i = 0; i < boxes_num; i++) {
        ...
        for (j = 0; j < pts_num; j++) {
            ...
            if (cur_in_flag) {
                if (cnt < sampled_pts_num) {
                    ...
                    for (k = 0; k < 3; k++)
                        pooled_pts_flat[temp_idx + k] = pts_flat[j * 3 + k];
                    temp_idx = i * sampled_pts_num * feature_len +
                               cnt * feature_len;
                    for (k = 0; k < feature_len; k++)
                        pooled_features_flat[temp_idx + k] =
                                pts_feature_flat[j * feature_len + k];
                    cnt++;
                } else
                    break;
            }
        ...
        if (cnt == 0) {
            ...
            for (j = cnt; j < sampled_pts_num; j++) {
                ...
                for (k = 0; k < 3; k++)
                    pooled_pts_flat[temp_idx + k] =
                            pooled_pts_flat[duplicate_idx + k];
                temp_idx = i * sampled_pts_num * feature_len + j * feature_len;
                duplicate_idx = i * sampled_pts_num * feature_len +
                                (j % cnt) * feature_len;
                for (k = 0; k < feature_len; k++) {
                    pooled_features_flat[temp_idx + k] =
                            pooled_features_flat[duplicate_idx + k];
                }
            }
        }
    }
