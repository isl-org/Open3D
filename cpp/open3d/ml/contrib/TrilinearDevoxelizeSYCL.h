...
// state between points, so we use a flat nd_range over (batch, point),
// flattened to 1-D via core::ParallelFor, rather than the CUDA
// grid-stride-over-one-block-per-batch layout or a bare sycl::range.
...
inline void TrilinearDevoxelizeSYCL(sycl::queue& queue,
    ...
    core::ParallelFor(queue, static_cast<int64_t>(b) * n, [=](int64_t idx) {
        const int batch_index = static_cast<int>(idx / n);
        ...
        for (int j = 0; j < c; ++j) {
            const int jr3 = j * r3;
            out[j * n + i] =
                    wgt000 * ft[jr3 + idx000] + wgt001 * ft[jr3 + idx001] +
                    wgt010 * ft[jr3 + idx010] + wgt011 * ft[jr3 + idx011] +
                    wgt100 * ft[jr3 + idx100] + wgt101 * ft[jr3 + idx101] +
                    wgt110 * ft[jr3 + idx110] + wgt111 * ft[jr3 + idx111];
        }
    });
}
...
inline void TrilinearDevoxelizeGradSYCL(sycl::queue& queue,
    ...
    core::ParallelFor(queue, static_cast<int64_t>(b) * n, [=](int64_t idx64) {
        const int batch_index = static_cast<int>(idx64 / n);
        const int i = static_cast<int>(idx64 % n);
        ...
        const int idx[8] = {ind[i],         ind[i + n],     ind[i + n * 2],
                            ind[i + n * 3], ind[i + n * 4], ind[i + n * 5],
                            ind[i + n * 6], ind[i + n * 7]};
        const float w[8] = {wgt[i],         wgt[i + n],     wgt[i + n * 2],
                            wgt[i + n * 3], wgt[i + n * 4], wgt[i + n * 5],
                            wgt[i + n * 6], wgt[i + n * 7]};
