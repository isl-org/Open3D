...
__global__ void l2SelectMinK(T* productDistances,
    ...
    for (int i = threadIdx.x; i < k; i += blockDim.x) {
        outDistances[row * dim + i] = smemK[i];
        outIndices[row * dim + i] = smemV[i];
    }
}
...
void runL2SelectMin(const cudaStream_t stream,
    ...
    // FAISS_ASSERT(centroidDistances.getSize(0) ==
    // productDistances.getSize(1)); FAISS_ASSERT(outDistances.getSize(1) == k);
