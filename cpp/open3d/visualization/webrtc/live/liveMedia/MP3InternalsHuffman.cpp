...
MP3HuffmanEncodingInfo::~MP3HuffmanEncodingInfo() { delete[] decodedValues; }
...
void updateSideInfoForHuffman(MP3SideInfo& sideInfo,
    ...
    /* Process granule 0 */
    {
        ...
        MP3HuffmanDecode(gr, isMPEG2, mainDataPtr, 0, origTotABsize, sfLength,
                         hei);

        /* Begin by computing new sizes for parts a & b (& their truncations) */
#ifdef DEBUG
        fprintf(stderr, "usifh-0: %d, %d:%d, %d:%d, %d:%d, %d:%d, %d:%d\n",
                hei.numSamples, sfLength / 8, sfLength % 8, hei.reg1Start / 8,
                hei.reg1Start % 8, hei.reg2Start / 8, hei.reg2Start % 8,
                hei.bigvalStart / 8, hei.bigvalStart % 8, origTotABsize / 8,
                origTotABsize % 8);
#endif
        if (p23L0 < sfLength) {
        ...
        if (origTotABsize > p23L0) {
            ...
            part23Length0aTruncation =
                    (truncation * (part23Length0a - sfLength)) /
        ...
        for (j = i; j < (int)hei.numSamples; ++j) {
            if (hei.allBitOffsets[j] ==
                part23Length0a + part23Length0aTruncation + part23Length0b)
                break;
            else if (hei.allBitOffsets[j] > part23Length0a +
                                                    part23Length0aTruncation +
                                                    part23Length0b) {
                --j;
                break;
        ...
        if (j < 0) { /* should happen only if we couldn't fit sfLength */
            ...
            adjustment = part23Length0a + part23Length0aTruncation +
                         part23Length0b - hei.allBitOffsets[j];
        }
#ifdef DEBUG
    ...
    if (isMPEG2) {
        ...
        if (origTotABsize > p23L1) {
            ...
            part23Length1aTruncation =
                    (truncation * (part23Length1a - sfLength)) /
                    (origTotABsize - sfLength);
        ...
        for (j = i; j < (int)hei.numSamples; ++j) {
            if (hei.allBitOffsets[j] ==
                part23Length1a + part23Length1aTruncation + part23Length1b)
                break;
            else if (hei.allBitOffsets[j] > part23Length1a +
                                                    part23Length1aTruncation +
                                                    part23Length1b) {
                --j;
                break;
        ...
        if (j < 0) { /* should happen only if we couldn't fit sfLength */
            ...
            adjustment = part23Length1a + part23Length1aTruncation +
                         part23Length1b - hei.allBitOffsets[j];
        }
#ifdef DEBUG
    ...
    fprintf(stderr, "usifh-end, new vals: %d (%d), %d (%d), %d (%d), %d (%d)\n",
            part23Length0a, part23Length0aTruncation, part23Length0b,
            part23Length0bTruncation, part23Length1a, part23Length1aTruncation,
            part23Length1b, part23Length1bTruncation);
#endif
}
...
static int read_decoder_table(unsigned char* fi) {
    ...
    for (n = 0; n < HTN; n++) {
        ...
        sscanf(line, "%s %s %u %u %u %u", command, rsf_ht[n].tablename,
               &rsf_ht[n].treelen, &rsf_ht[n].xlen, &rsf_ht[n].ylen,
               &rsf_ht[n].linbits);
        ...
        if (strcmp(command, ".reference") == 0) {
            ...
            rsf_ht[n].ref = -1;
            rsf_ht[n].val = (unsigned char(*)[2]) new unsigned char
                    [2 * (rsf_ht[n].treelen)];
...
static unsigned char const stab[3][6][4] = {{{6, 5, 5, 5},
                                             {6, 5, 7, 3},
                                             {11, 10, 0, 0},
                                             {7, 7, 7, 0},
                                             {6, 6, 6, 3},
...
static int rsf_huffman_decoder(
        BitVector& bv,
    ...
    *x = *y = *v = *w = 0;
    ...
    /* Process sign and escape encodings for dual tables. */
        ...
        if (h->linbits)
            if ((h->xlen - 1) == (unsigned)*x) *x += bv.getBits(h->linbits);
        if (*x)
            if (bv.get1Bit() == 1) *x = -*x;
        if (h->linbits)
            if ((h->ylen - 1) == (unsigned)*y) *y += bv.getBits(h->linbits);
        if (*y)
            if (bv.get1Bit() == 1) *y = -*y;
    }
...
inline int getNextSample(unsigned char const*& fromPtr) {
    int sample
            ...
            = (fromPtr[0] << 24) | (fromPtr[1] << 16) | (fromPtr[2] << 8) |
              fromPtr[3];
#else
...
static void buildHuffmanEncodingTable(struct huffcodetab* h) {
    ...
    unsigned powerOf2 = 1;
    for (unsigned bitsLength = 1; bitsLength <= 8 * SIZEOF_HUFFBITS;
         ++bitsLength) {
        powerOf2 *= 2;
...
static void rsf_huffman_encoder(
        BitVector& bv, struct huffcodetab* h, int x, int y, int v, int w) {
...
#ifdef DEBUG
            fprintf(stderr,
                    "rsf_huffman_encoder quad sanity check fails: "
                    "%x,%x,%x,%x\n",
                    x, y, v, w);
#endif
        }
...
#ifdef DEBUG
            fprintf(stderr,
                    "rsf_huffman_encoder dual sanity check 1 fails: "
                    "%x,%x,%x,%x\n",
                    x, y, v, w);
#endif
        }
...
#ifdef DEBUG
            fprintf(stderr,
                    "rsf_huffman_encoder dual sanity check 2 fails: "
                    "%x,%x,%x,%x\n",
                    x, y, v, w);
#endif
        }
