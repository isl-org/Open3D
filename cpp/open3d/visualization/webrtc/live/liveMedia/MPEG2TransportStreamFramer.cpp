...
// A filter that passes through (unchanged) chunks that contain an integral
// number of MPEG-2 Transport Stream packets, but returning (in
// "fDurationInMicroseconds") an updated estimate of the time gap between
...
// packet transmission times remains matched with the PCR times (which will be
// the times that we expect receivers to play the incoming packets). (must be <=
// 1)
#endif
...
public:
    PIDStatus(double _firstClock, double _firstRealTime)
        : firstClock(_firstClock),
          lastClock(_firstClock),
          firstRealTime(_firstRealTime),
          lastRealTime(_firstRealTime),
          lastPacketNum(0) {}
...
MPEG2TransportStreamFramer* MPEG2TransportStreamFramer ::createNew(
...
MPEG2TransportStreamFramer ::MPEG2TransportStreamFramer(
    ...
    : FramedFilter(env, inputSource),
      fTSPacketCount(0),
      fTSPacketDurationEstimate(0.0),
      fTSPCRCount(0),
      fLimitNumTSPacketsToStream(False),
      fNumTSPacketsToStream(0),
      fLimitTSPacketsToStreamByPCR(False),
      fPCRLimit(0.0) {
...
void MPEG2TransportStreamFramer::setNumTSPacketsToStream(
        unsigned long numTSRecordsToStream) {
...
void MPEG2TransportStreamFramer ::afterGettingFrame(
        void* clientData,
        unsigned frameSize,
    ...
    MPEG2TransportStreamFramer* framer =
            (MPEG2TransportStreamFramer*)clientData;
    framer->afterGettingFrame1(frameSize, presentationTime);
}

#define TRANSPORT_SYNC_BYTE 0x47

void MPEG2TransportStreamFramer::afterGettingFrame1(
    ...
    fFrameSize = numTSPackets *
                 TRANSPORT_PACKET_SIZE;  // an integral # of TS packets
    if (fFrameSize == 0) {
        // We didn't read a complete TS packet; assume that the input source has
        // closed.
        handleClosure();
        return;
    }
    ...
    for (syncBytePosition = 0; syncBytePosition < fFrameSize;
         ++syncBytePosition) {
    ...
    if (syncBytePosition == fFrameSize) {
        ...
        // There's a sync byte, but not at the start of the data.  Move the good
        // data to the start of the buffer, then read more to fill it up again:
    ...
    for (unsigned i = 0; i < numTSPackets; ++i) {
        if (!updateTSPacketDurationEstimate(&fTo[i * TRANSPORT_PACKET_SIZE],
                                            timeNow)) {
            // We hit a preset limit (based on PCR) within the stream.  Handle
            // this as if the input source has closed:
            handleClosure();
            return;
        }
    }
...
Boolean MPEG2TransportStreamFramer::updateTSPacketDurationEstimate(
    ...
    u_int32_t pcrBaseHigh =
            (pkt[6] << 24) | (pkt[7] << 16) | (pkt[8] << 8) | pkt[9];
    ...
    PIDStatus* pidStatus =
            (PIDStatus*)(fPIDStatusTable->Lookup(reinterpret_cast<char*>(pid)));

    if (pidStatus == NULL) {
        // We're seeing this PID's PCR for the first time:
        pidStatus = new PIDStatus(clock, timeNow);
        fPIDStatusTable->Add(reinterpret_cast<char*>(pid), pidStatus);
#ifdef DEBUG_PCR
        fprintf(stderr,
                "PID 0x%x, FIRST PCR 0x%08x+%d:%03x == %f @ %f, pkt #%lu\n",
                pid, pcrBaseHigh, pkt[10] >> 7, pcrExt, clock, timeNow,
                fTSPacketCount);
#endif
    } else {
        // We've seen this PID's PCR before; update our per-packet duration
        // estimate:
        int64_t packetsSinceLast =
                (int64_t)(fTSPacketCount - pidStatus->lastPacketNum);
        // it's "int64_t" because some compilers can't convert "u_int64_t" ->
        // "double"
        double durationPerPacket =
                (clock - pidStatus->lastClock) / packetsSinceLast;

        // Hack (suggested by "Romain"): Don't update our estimate if this PCR
        // appeared unusually quickly. (This can produce more accurate estimates
        // for wildly VBR streams.)
        ...
        if (fTSPacketDurationEstimate == 0.0) {  // we've just started
            ...
            if (transmitDuration > playoutDuration) {
                fTSPacketDurationEstimate *=
                        TIME_ADJUSTMENT_FACTOR;  // reduce estimate
            } else if (transmitDuration + MAX_PLAYOUT_BUFFER_DURATION <
                       playoutDuration) {
                fTSPacketDurationEstimate /=
                        TIME_ADJUSTMENT_FACTOR;  // increase estimate
            }
        } else {
            // the PCR has a discontinuity from its previous value; don't use it
            // now, but reset our PCR and real-time values to compensate:
            pidStatus->firstClock = clock;
            pidStatus->firstRealTime = timeNow;
        }
#ifdef DEBUG_PCR
        fprintf(stderr,
                "PID 0x%x, PCR 0x%08x+%d:%03x == %f @ %f (diffs %f @ %f), pkt "
                "#%lu, discon %d => this duration %f, new estimate %f, mean "
                "PCR period=%f\n",
                pid, pcrBaseHigh, pkt[10] >> 7, pcrExt, clock, timeNow,
                clock - pidStatus->firstClock,
                timeNow - pidStatus->firstRealTime, fTSPacketCount,
                discontinuity_indicator != 0, durationPerPacket,
                fTSPacketDurationEstimate, meanPCRPeriod);
#endif
    }
