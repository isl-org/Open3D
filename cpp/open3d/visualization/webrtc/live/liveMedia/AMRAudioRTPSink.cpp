...
AMRAudioRTPSink* AMRAudioRTPSink::createNew(UsageEnvironment& env,
    ...
    return new AMRAudioRTPSink(env, RTPgs, rtpPayloadFormat, sourceIsWideband,
                               numChannelsInSource);
}

AMRAudioRTPSink ::AMRAudioRTPSink(UsageEnvironment& env,
                                  Groupsock* RTPgs,
                                  unsigned char rtpPayloadFormat,
                                  Boolean sourceIsWideband,
                                  unsigned numChannelsInSource)
    : AudioRTPSink(env,
                   ...
                   numChannelsInSource),
      fSourceIsWideband(sourceIsWideband),
      fFmtpSDPLine(NULL) {}

AMRAudioRTPSink::~AMRAudioRTPSink() { delete[] fFmtpSDPLine; }
...
Boolean AMRAudioRTPSink::sourceIsCompatibleWithUs(MediaSource& source) {
    ...
    if (amrSource.numChannels() > 1) {
        envir() << "AMRAudioRTPSink: Warning: Input source has "
                << amrSource.numChannels()
                << " audio channels.  In the current implementation, the "
                   "multi-frame frame-block will be split over multiple RTP "
                   "packets\n";
    }
...
void AMRAudioRTPSink::doSpecialFrameHandling(
...
Boolean AMRAudioRTPSink ::frameCanAppearAfterPacketStart(
