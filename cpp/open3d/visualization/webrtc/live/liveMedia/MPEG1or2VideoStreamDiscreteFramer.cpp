...
MPEG1or2VideoStreamDiscreteFramer* MPEG1or2VideoStreamDiscreteFramer::createNew(
    ...
    return new MPEG1or2VideoStreamDiscreteFramer(
            env, inputSource, iFramesOnly, vshPeriod,
...
void MPEG1or2VideoStreamDiscreteFramer ::afterGettingFrame(
        void* clientData,
        unsigned frameSize,
...
void MPEG1or2VideoStreamDiscreteFramer ::afterGettingFrame1(
        unsigned frameSize,
        unsigned numTruncatedBytes,
        struct timeval presentationTime,
        unsigned durationInMicroseconds) {
    ...
    if (frameSize >= 4 && fTo[0] == 0 && fTo[1] == 0 && fTo[2] == 1) {
        fPictureEndMarker =
                True;  // Assume that we have a complete 'picture' here
        ...
        if (nextCode == 0xB3) {  // VIDEO_SEQUENCE_HEADER_START_CODE
            ...
            // Also, save away this Video Sequence Header, in case we need it
            // later: First, figure out how big it is:
            unsigned vshSize;
            for (vshSize = 4; vshSize < frameSize - 3; ++vshSize) {
                if (fTo[vshSize] == 0 && fTo[vshSize + 1] == 0 &&
                    fTo[vshSize + 2] == 1 &&
                    (fTo[vshSize + 3] == 0xB8 || fTo[vshSize + 3] == 0x00))
                    break;
            }
            if (vshSize == frameSize - 3)
                vshSize = frameSize;  // There was nothing else following it
            if (vshSize <= sizeof fSavedVSHBuffer) {
                memmove(fSavedVSHBuffer, fTo, vshSize);
                fSavedVSHSize = vshSize;
                fSavedVSHTimestamp = presentationTime.tv_sec +
                                     presentationTime.tv_usec / (double)MILLION;
            }
        } else if (nextCode == 0xB8) {  // GROUP_START_CODE
            // If necessary, insert a saved Video Sequence Header in front of
            // this:
            double pts = presentationTime.tv_sec +
                         presentationTime.tv_usec / (double)MILLION;
            if (pts > fSavedVSHTimestamp + fVSHPeriod &&
                fSavedVSHSize + frameSize <= fMaxSize) {
                memmove(&fTo[fSavedVSHSize], &fTo[0],
                        frameSize);  // make room for the header
        ...
        if (nextCode == 0x00 /*PICTURE_START_CODE*/ && i + 2 < frameSize) {
            ...
            // If this is not an "I" frame, but we were asked for "I" frames
            // only, then try again:
            ...
            if (!fLeavePresentationTimesUnmodified &&
                ...
                unsigned usIncrement =
                        fFrameRate == 0.0 ? 0
                                          : (unsigned)((trIncrement * MILLION) /
                                                       fFrameRate);
