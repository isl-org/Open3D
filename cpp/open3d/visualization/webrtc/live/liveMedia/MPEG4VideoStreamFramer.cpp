...
private:
    ...
    unsigned fSecondsSinceLastTimeCode, fTotalTicksSinceLastTimeCode,
            fPrevNewTotalTicks;
...
MPEG4VideoStreamFramer* MPEG4VideoStreamFramer::createNew(
...
MPEG4VideoStreamFramer::MPEG4VideoStreamFramer(UsageEnvironment& env,
    ...
    fParser =
            createParser ? new MPEG4VideoStreamParser(this, inputSource) : NULL;
}
...
MPEG4VideoStreamParser ::MPEG4VideoStreamParser(
        MPEG4VideoStreamFramer* usingSource, FramedSource* inputSource)
...
void MPEG4VideoStreamParser::flushInput() {
    ...
    if (fCurrentParseState != PARSING_VISUAL_OBJECT_SEQUENCE) {
        setParseState(PARSING_VISUAL_OBJECT_SEQUENCE);  // later, change to GOV
                                                        // or VOP? #####
    }
}
...
#ifdef DEBUG
        fprintf(stderr,
                "MPEG4VideoStreamParser::parse() EXCEPTION (This is normal "
...
unsigned MPEG4VideoStreamParser ::parseVisualObjectSequence(
        Boolean haveSeenStartCode) {
#ifdef DEBUG
...
unsigned MPEG4VideoStreamParser::parseVisualObject() {
...
#ifdef DEBUG
        fprintf(stderr,
                "visual_object_verid: 0x%x; visual_object_priority: 0x%x\n",
    ...
    if (visual_object_type != 1) {
        usingSource()->envir()
                << "MPEG4VideoStreamParser::parseVisualObject(): Warning: We "
                   "don't handle visual_object_type "
                << visual_object_type << "\n";
    }
...
void MPEG4VideoStreamParser::analyzeVOLHeader() {
    ...
    do {
        ...
        fNumVTIRBits = 0;
        for (unsigned test = vop_time_increment_resolution; test > 0;
             test /= 2) {
            ++fNumVTIRBits;
        }
        ...
        if (fixed_vop_rate) {
            ...
            if (fixed_vop_time_increment == 0) {
                usingSource()->envir()
                        << "MPEG4VideoStreamParser::analyzeVOLHeader(): "
                           "fixed_vop_time_increment is zero!\n";
            }
#endif
        }
...
unsigned MPEG4VideoStreamParser::parseVideoObjectLayer() {
    ...
    if (!isVideoObjectLayerStartCode(next4Bytes)) {
        usingSource()->envir()
                << "MPEG4VideoStreamParser::parseVideoObjectLayer(): This "
                   "appears to be a 'short video header', which we currently "
                   "don't support\n";
    }
...
unsigned MPEG4VideoStreamParser::parseGroupOfVideoObjectPlane() {
...
#if defined(DEBUG) || defined(DEBUG_TIMESTAMPS)
    fprintf(stderr,
            "time_code: 0x%05x, hours %d, minutes %d, marker_bit %d, seconds "
            "%d\n",
            time_code, time_code_hours, time_code_minutes, marker_bit,
            time_code_seconds);
#endif
    ...
    // Note: Because the GOV header can appear anywhere (not just at a 1s
    // point), we don't pass "fTotalTicksSinceLastTimeCode" as the
    // "picturesSinceLastGOP" parameter.
    fSecondsSinceLastTimeCode = 0;
...
unsigned MPEG4VideoStreamParser::parseVideoObjectPlane() {
    ...
    if ((timeInfo & mask) == 0) {
        usingSource()->envir()
                << "MPEG4VideoStreamParser::parseVideoObjectPlane(): marker "
                   "bit not set!\n";
    }
    mask >>= 1;
    ...
    if ((mask >> (fNumVTIRBits - 1)) == 0) {
        usingSource()->envir()
                << "MPEG4VideoStreamParser::parseVideoObjectPlane(): 32-bits "
                   "are not enough to get \"vop_time_increment\"!\n";
    }
...
#ifdef DEBUG
    fprintf(stderr,
            "vop_coding_type: %d(%c), modulo_time_base: %d, "
            "vop_time_increment: %d\n",
            vop_coding_type, "IPBS"[vop_coding_type], modulo_time_base,
            vop_time_increment);
#endif
    ...
    if (fixed_vop_time_increment > 0) {
        ...
        if (vop_time_increment > 0 || modulo_time_base > 0) {
            ...
            // Note: "fSecondsSinceLastTimeCode" and "fPrevNewTotalTicks" are
            // not used.
        }
    } else {
        ...
        if (newTotalTicks == fPrevNewTotalTicks && fPrevNewTotalTicks > 0) {
...
#ifdef DEBUG
            fprintf(stderr,
                    "Buggy MPEG-4 video stream: \"vop_time_increment\" did not "
                    "change!\n");
#endif
            // The following assumes that we don't have 'B' frames.  If we do,
            // then TARFU!
            ...
            if (newTotalTicks < fPrevNewTotalTicks && vop_coding_type != 2 /*B*/
                ...
                // "modulo_time_base" changing (or just having had a new time
                // code). Overcome this by pretending that "vop_time_increment"
                // *did* wrap around:
#ifdef DEBUG
                fprintf(stderr,
                        "Buggy MPEG-4 video stream: \"vop_time_increment\" "
                        "wrapped around, but without \"modulo_time_base\" "
                        "changing!\n");
#endif
            ...
            if (vop_coding_type != 2 /*B*/) {
                int pictureCountDelta =
                        newTotalTicks - fTotalTicksSinceLastTimeCode;
    ...
    switch (next4Bytes) {
        ...
        default: {
            if (isVideoObjectStartCode(next4Bytes)) {
                ...
                // copy all bytes that we see, up until we reach a
                // VOP_START_CODE:
                u_int32_t next4Bytes = get4Bytes();
                while (next4Bytes != VOP_START_CODE) {
                    saveToNextCode(next4Bytes);
