...
public:
    MPEGProgramStreamParser(MPEG1or2Demux* usingDemux,
...
MPEG1or2Demux ::MPEG1or2Demux(UsageEnvironment& env,
    ...
    : Medium(env),
      ...
      fMPEGversion(0),
      fNextAudioStreamNumber(0),
      fNextVideoStreamNumber(0),
      fReclaimWhenLastESDies(reclaimWhenLastESDies),
      fNumOutstandingESs(0),
      fNumPendingReads(0),
      fHaveUndeliveredData(False) {
...
void MPEG1or2Demux ::noteElementaryStreamDeletion(
        MPEG1or2DemuxedElementaryStream* /*es*/) {
    if (--fNumOutstandingESs == 0 && fReclaimWhenLastESDies) {
        Medium::close(this);
    }
}

void MPEG1or2Demux::flushInput() { fParser->flushInput(); }

MPEG1or2DemuxedElementaryStream* MPEG1or2Demux::newElementaryStream(
...
// Appropriate one of the reserved stream id tags to mean: return raw PES
// packets:
#define RAW_PES 0xFC
...
void MPEG1or2Demux::registerReadInterest(
    ...
    if (out.isCurrentlyAwaitingData) {
        envir() << "MPEG1or2Demux::registerReadInterest(): attempt to read "
...
Boolean MPEG1or2Demux::useSavedData(
        u_int8_t streamIdTag,
        unsigned char* to,
        unsigned maxSize,
        FramedSource::afterGettingFunc* afterGettingFunc,
...
void MPEG1or2Demux::continueReadProcessing() {
    while (fNumPendingReads > 0) {
        ...
        if (acquiredStreamIdTag != 0) {
            ...
            // 'leaf' source, we can call this directly, without risking
            // infinite recursion.
            if (newOut.fAfterGettingFunc != NULL) {
                (*newOut.fAfterGettingFunc)(
                        newOut.afterGettingClientData, newOut.frameSize,
                        0 /* numTruncatedBytes */, newOut.presentationTime,
                        0 /* durationInMicroseconds ?????#####*/);
                --fNumPendingReads;
            }
        } else {
...
void MPEG1or2Demux::getNextFrame(
        u_int8_t streamIdTag,
        unsigned char* to,
        unsigned maxSize,
        FramedSource::afterGettingFunc* afterGettingFunc,
    ...
    if (useSavedData(streamIdTag, to, maxSize, afterGettingFunc,
                     afterGettingClientData)) {
        return;
    }

    // Then save the parameters of the specified stream id:
    registerReadInterest(streamIdTag, to, maxSize, afterGettingFunc,
                         afterGettingClientData, onCloseFunc,
                         onCloseClientData);

    // Next, if we're the only currently pending read, continue looking for
    // data:
    ...
    for (i = 0; i < 256; ++i) {
        ...
        if (out.isCurrentlyAwaitingData) {
            if (out.fOnCloseFunc != NULL) {
                ...
                savedPending[numPending].onCloseClientData =
                        out.onCloseClientData;
                ++numPending;
            }
        }
        delete out.savedDataHead;
        out.savedDataHead = out.savedDataTail = NULL;
        out.savedDataTotalSize = 0;
        out.isPotentiallyReadable = out.isCurrentlyActive =
                out.isCurrentlyAwaitingData = False;
    }
...
MPEGProgramStreamParser::MPEGProgramStreamParser(MPEG1or2Demux* usingDemux,
    ...
    : StreamParser(inputSource,
                   MPEG1or2Demux::handleClosure,
                   usingDemux,
                   &MPEG1or2Demux::continueReadProcessing,
                   usingDemux),
      fUsingDemux(usingDemux),
      fCurrentParseState(PARSING_PACK_HEADER) {}
...
#ifdef DEBUG
        fprintf(stderr,
                "MPEGProgramStreamParser::parse() EXCEPTION (This is normal "
...
void MPEGProgramStreamParser::parsePackHeader() {
    ...
    while (1) {
...
#ifdef DEBUG
            fprintf(stderr,
                    "found packet start code 0x%02x instead of pack header\n",
                    first4Bytes);
#endif
        ...
        setParseState(
                PARSING_PACK_HEADER);  // ensures we progress over bad data
        if ((first4Bytes & 0xFF) >
            1) {  // a system code definitely doesn't start here
            skipBytes(4);
        } else {
            skipBytes(1);
        }
...
void MPEGProgramStreamParser::parseSystemHeader() {
    ...
    // According to the MPEG-1 and MPEG-2 specs, "remaining_header_length"
    // should be at least 6 bytes.  Check this now:
    if (remaining_header_length < 6) {
        fUsingDemux->envir() << "StreamParser::parseSystemHeader(): saw "
                                "strange header_length: "
...
unsigned char MPEGProgramStreamParser::parsePESPacket() {
    ...
    // Parse over the rest of the header, until we get to the packet data
    // itself. This varies depending upon the MPEG version:
    ...
    if (stream_id == RAW_PES) {
        restoreSavedParserState();  // so we deliver from the beginning of the
                                    // PES packet
    ...
    if (PES_packet_length < bytesSkipped) {
        fUsingDemux->envir()
                << "StreamParser::parsePESPacket(): saw inconsistent "
                   "PES_packet_length "
        ...
        MPEG1or2Demux::OutputDescriptor_t& out =
                fUsingDemux->fOutput[stream_id];
        if (out.isCurrentlyAwaitingData) {
            unsigned numBytesToCopy;
            if (PES_packet_length > out.maxSize) {
                fUsingDemux->envir()
                        << "MPEGProgramStreamParser::parsePESPacket() error: "
                           "PES_packet_length ("
                        << PES_packet_length
                        << ") exceeds max frame size asked for (" << out.maxSize
                        << ")\n";
...
#ifdef DEBUG
            fprintf(stderr,
                    "%d, %d bytes of PES_packet_data (out.maxSize: %d); first "
                    "4 bytes: 0x%08x\n",
                    frameCount, numBytesToCopy, out.maxSize, next4Bytes);
            fflush(stderr);
#endif
...
#ifdef DEBUG
            fprintf(stderr,
                    "%d, currently undeliverable PES data; first 4 bytes: "
                    "0x%08x - currently undeliverable!\n",
                    frameCount, next4Bytes);
            fflush(stderr);
#endif
            ...
            throw READER_NOT_READY;
                   ...
                   out.savedDataTotalSize + PES_packet_length <
                           1000000 /*limit*/) {
            // Someone is interested in this stream, but hasn't begun reading it
            // yet. Save this data, so that the reader will get it when he later
            // asks for it.
            ...
            MPEG1or2Demux::OutputDescriptor::SavedData* savedData =
                    new MPEG1or2Demux::OutputDescriptor::SavedData(
                            buf, PES_packet_length);
