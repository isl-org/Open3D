...
public:
    AVIIndexRecord(unsigned chunkId,
                   unsigned flags,
                   unsigned offset,
                   unsigned size)
        : fNext(NULL),
...
AVIFileSink* AVIFileSink ::createNew(UsageEnvironment& env,
                                     ...
                                     unsigned movieFPS,
                                     Boolean packetLossCompensate) {
    AVIFileSink* newSink = new AVIFileSink(env, inputSession, outputFileName,
                                           bufferSize, movieWidth, movieHeight,
                                           movieFPS, packetLossCompensate);
...
void AVIFileSink ::afterGettingFrame(void* clientData,
    ...
    if (numTruncatedBytes > 0) {
        ioState->envir() << "AVIFileSink::afterGettingFrame(): The input frame "
                            "data was too large for our buffer.  "
                         << numTruncatedBytes
...
void AVIFileSink::completeOutputFile() {
    ...
    //// Global fields:
    add4ByteString("idx1");
    addWord(fNumIndexRecords * 4 *
            4);  // the size of all of the index records, which come next:
    for (AVIIndexRecord* indexRecord = fIndexRecordsHead; indexRecord != NULL;
...
void AVISubsessionIOState::setAVIstate(unsigned subsessionIndex) {
    ...
    if (fIsVideo) {
        fAVISubsessionTag = fourChar('0' + subsessionIndex / 10,
                                     '0' + subsessionIndex % 10, 'd', 'c');
        if (strcmp(fOurSubsession.codecName(), "JPEG") == 0) {
            ...
            fAVICodecHandlerType =
                    fourChar('m', 'p', 'g', '1');  // what about MPEG-2?
        ...
        fAVISize =
                fOurSink.fMovieWidth * fOurSink.fMovieHeight * 3;  // ??? #####
    } else if (fIsAudio) {
        fIsByteSwappedAudio = False;  // by default
        fAVISubsessionTag = fourChar('0' + subsessionIndex / 10,
                                     '0' + subsessionIndex % 10, 'w', 'b');
        fAVICodecHandlerType = 1;  // ??? ####
        unsigned numChannels = fOurSubsession.numChannels();
        fAVISamplingFrequency =
                fOurSubsession.rtpTimestampFrequency();  // default
        if (strcmp(fOurSubsession.codecName(), "L16") == 0) {
            fIsByteSwappedAudio =
                    True;  // need to byte-swap data before writing it
            fWAVCodecTag = 0x0001;
...
void AVISubsessionIOState::useFrame(SubsessionBuffer& buffer) {
    ...
    if (fPrevPresentationTime.tv_usec != 0 ||
        ...
        if (uSecondsDiff > 0) {
            unsigned bytesPerSecond =
                    (unsigned)((frameSize * 1000000.0) / uSecondsDiff);
    ...
    AVIIndexRecord* newIndexRecord = new AVIIndexRecord(
            fAVISubsessionTag,              // chunk id
...
#define addFileHeader(tag, name)                                               \
    unsigned AVIFileSink::addFileHeader_##name() {                             \
        add4ByteString("" #tag "");                                            \
        unsigned headerSizePosn = (unsigned)TellFile64(fOutFid);               \
        addWord(0);                                                            \
        add4ByteString("" #name "");                                           \
...
#define addFileHeader1(name)                                     \
    unsigned AVIFileSink::addFileHeader_##name() {               \
        ...
        unsigned headerSizePosn = (unsigned)TellFile64(fOutFid); \
        addWord(0);                                              \
        unsigned ignoredSize =                                   \
                8; /*don't include size of name or size fields*/ \
        unsigned size = 8
...
size += addWord(AVIF_TRUSTCKTYPE | AVIF_HASINDEX |
                AVIF_ISINTERLEAVED);  // dwFlags
...
addFileHeader1(strh);
size += add4ByteString(
        fCurrentIOState->fIsVideo
                ? "vids"
                : fCurrentIOState->fIsAudio ? "auds" : "????");  // fccType
