...
MPEG1or2VideoStreamFramer::MPEG1or2VideoStreamFramer(UsageEnvironment& env,
    ...
    fParser = createParser ? new MPEG1or2VideoStreamParser(
                                     this, inputSource, iFramesOnly, vshPeriod)
                           : NULL;
}

MPEG1or2VideoStreamFramer::~MPEG1or2VideoStreamFramer() {}

MPEG1or2VideoStreamFramer* MPEG1or2VideoStreamFramer::createNew(
        UsageEnvironment& env,
        FramedSource* inputSource,
        Boolean iFramesOnly,
        double vshPeriod) {
    // Need to add source type checking here???  #####
    return new MPEG1or2VideoStreamFramer(env, inputSource, iFramesOnly,
                                         vshPeriod);
}
...
#ifdef DEBUG
        fprintf(stderr,
                "MPEG1or2VideoStreamParser::parse() EXCEPTION (This is normal "
...
unsigned MPEG1or2VideoStreamParser ::parseVideoSequenceHeader(
    ...
    fprintf(stderr,
            "horizontal_size_value: %d, vertical_size_value: %d, "
            "aspect_ratio_information: %d, frame_rate_code: %d (=>%f fps), "
            "bit_rate_value: %d (=>%d bps), vbv_buffer_size_value: %d\n",
            horizontal_size_value, vertical_size_value,
            aspect_ratio_information, frame_rate_code,
            usingSource()->fFrameRate, bit_rate_value, bit_rate_value * 400,
    ...
    } while (next4Bytes != GROUP_START_CODE &&
             next4Bytes != PICTURE_START_CODE);
...
#if defined(DEBUG) || defined(DEBUG_TIMESTAMPS)
    fprintf(stderr,
            "time_code: 0x%07x, drop_frame %d, hours %d, minutes %d, seconds "
            "%d, pictures %d\n",
            time_code, drop_frame_flag, time_code_hours, time_code_minutes,
...
unsigned MPEG1or2VideoStreamParser::parsePictureHeader() {
    ...
    fprintf(stderr,
            "temporal_reference: %d, picture_coding_type: %d, vbv_delay: %d\n",
