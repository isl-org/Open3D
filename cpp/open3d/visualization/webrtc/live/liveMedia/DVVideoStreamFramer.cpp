...
// A filter that parses a DV input stream into DV frames to deliver to the
// downstream object Implementation (Thanks to Ben Hutchings for his help,
...
DVVideoStreamFramer::DVVideoStreamFramer(
    ...
    : FramedFilter(env, inputSource),
      ...
      fSourceIsSeekable(sourceIsSeekable) {
...
DVVideoStreamFramer* DVVideoStreamFramer::createNew(
        UsageEnvironment& env,
        FramedSource* inputSource,
        Boolean sourceIsSeekable,
        Boolean leavePresentationTimesUnmodified) {
    return new DVVideoStreamFramer(env, inputSource, sourceIsSeekable,
                                   leavePresentationTimesUnmodified);
}
...
Boolean DVVideoStreamFramer::getFrameParameters(unsigned& frameSize,
...
void DVVideoStreamFramer::getProfile() {
    // To determine the stream's profile, we need to first read a chunk of data
    // that we can parse:
    fInputSource->getNextFrame(fSavedInitialBlocks,
                               DV_SAVED_INITIAL_BLOCKS_SIZE, afterGettingFrame,
...
Boolean DVVideoStreamFramer::isDVVideoStreamFramer() const { return True; }

void DVVideoStreamFramer::doGetNextFrame() {
    fFrameSize = 0;  // initially, until we deliver data

    // If we have saved initial blocks (and won't be seeking back to re-read
    // this data), so use this data first.
    if (fInitialBlocksPresent && !fSourceIsSeekable) {
        // For simplicity, we require the downstream object's buffer to be >=
        // this data's size:
    ...
    // (But first, make sure that we read an integral multiple of the DV block
    // size.)
...
void DVVideoStreamFramer::getAndDeliverData() {
    unsigned const totFrameSize =
            fOurProfile != NULL
                    ? ((DVVideoProfile const*)fOurProfile)->dvFrameSize
                    : DV_SMALLEST_POSSIBLE_FRAME_SIZE;
    unsigned totBytesToDeliver =
            totFrameSize < fMaxSize ? totFrameSize : fMaxSize;
    unsigned numBytesToRead = totBytesToDeliver - fFrameSize;

    fInputSource->getNextFrame(fTo, numBytesToRead, afterGettingFrame, this,
                               FramedSource::handleClosure, this);
}

void DVVideoStreamFramer::afterGettingFrame(
        void* clientData,
        unsigned frameSize,
...
void DVVideoStreamFramer::afterGettingFrame(unsigned frameSize,
                                            unsigned numTruncatedBytes,
                                            struct timeval presentationTime) {
    if (fOurProfile == NULL && frameSize >= DV_SAVED_INITIAL_BLOCKS_SIZE) {
        // (Try to) parse this data enough to figure out its profile.
        // We assume that the data begins on a (80-byte) block boundary, but not
        // necessarily on a (150-block) sequence boundary. We therefore scan
        // each 80-byte block, until we find the 6-block header that begins a
        // sequence:
        ...
        for (u_int8_t const* ptr = data;
            ...
            if (sectionHeader == DV_SECTION_HEADER &&
                (packHeaderNum == DV_PACK_HEADER_10 ||
                 packHeaderNum == DV_PACK_HEADER_12) &&
                (sectionVAUX >= DV_SECTION_VAUX_MIN &&
                 sectionVAUX <= DV_SECTION_VAUX_MAX)) {
                // This data begins a sequence; look up the DV profile from
                // this:
                u_int8_t const apt = DVData(0, 1) & 0x07;
                u_int8_t const sType = DVData(5, 48) & 0x1F;
                u_int8_t const sequenceCount =
                        (packHeaderNum == DV_PACK_HEADER_10) ? 10 : 12;

                // Use these three parameters (apt, sType, sequenceCount) to
                // look up the DV profile:
                for (DVVideoProfile const* profile = profiles;
                     profile->name != NULL; ++profile) {
                    if (profile->apt == apt && profile->sType == sType &&
                        profile->sequenceCount == sequenceCount) {
                        fOurProfile = profile;
                        break;
                    }
                }
                break;  // because we found a correct sequence header (even if
                        // we don't happen to define a profile for it)
            }
        }
    }

    if (fTo != NULL) {  // There is a downstream object; complete delivery to it
                        // (or read more data, if necessary)
        unsigned const totFrameSize =
                fOurProfile != NULL
                        ? ((DVVideoProfile const*)fOurProfile)->dvFrameSize
                        : DV_SMALLEST_POSSIBLE_FRAME_SIZE;
        ...
        fTo += frameSize;
        fPresentationTime =
                presentationTime;  // by default; may get changed below

        if (fFrameSize < totFrameSize && fFrameSize < fMaxSize &&
            numTruncatedBytes == 0) {
            ...
            if (fOurProfile != NULL) {
                // Also set the presentation time, and increment it for next
                // time, based on the length of this frame:
                if (!fLeavePresentationTimesUnmodified)
                    fPresentationTime = fNextFramePresentationTime;

                DVVideoProfile const* ourProfile =
                        (DVVideoProfile const*)fOurProfile;
                double durationInMicroseconds =
                        (fFrameSize * ourProfile->frameDuration) /
                        ourProfile->dvFrameSize;
