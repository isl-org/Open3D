...
MP3FileSource::~MP3FileSource() { delete fStreamState; }

char const* MP3FileSource::MIMEtype() const { return "audio/MPEG"; }

MP3FileSource* MP3FileSource::createNew(UsageEnvironment& env,
                                        char const* fileName) {
...
unsigned MP3FileSource::fileSize() const { return fStreamState->fileSize(); }
...
void MP3FileSource::seekWithinFile(double seekNPT, double streamDuration) {
    ...
    // First, make sure that 0.0 <= seekNPT <= seekNPT + streamDuration <=
    // fileDuration
    if (seekNPT < 0.0) {
        seekNPT = 0.0;
    ...
    unsigned seekByteNumber =
            fStreamState->getByteNumberFromPositionFraction(seekFraction);
    ...
    if (streamDuration > 0.0) {
        ...
        unsigned endByteNumber =
                fStreamState->getByteNumberFromPositionFraction(endFraction);
...
void MP3FileSource::doGetNextFrame() {
    ...
    // that performs very badly (chewing up lots of CPU time, apparently
    // polling) on Windows.  Until this is fixed, we just call our
    // "afterGetting()" function directly.  This avoids infinite recursion, as
    // long as our sink is discontinuous, which is the case for the RTP sink
    // that liveCaster/lc uses. #####
    afterGetting(this);
#else
...
Boolean MP3FileSource::doGetNextFrame1() {
    ...
    if (!fStreamState->readFrame(fTo, fMaxSize, fFrameSize,
        ...
        char tmp[200];
        sprintf(tmp,
                "Insufficient buffer size %d for reading MPEG audio frame "
                "(needed %d)\n",
                fMaxSize, fFrameSize);
