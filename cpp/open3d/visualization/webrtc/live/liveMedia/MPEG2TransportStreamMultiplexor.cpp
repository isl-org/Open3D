...
#define PAT_PERIOD_IF_UNTIMED \
    100  // # of packets between Program Association Tables (if not timed)
#define PMT_PERIOD_IF_UNTIMED \
    500  // # of packets between Program Map Tables (if not timed)
...
MPEG2TransportStreamMultiplexor ::MPEG2TransportStreamMultiplexor(
    ...
    : FramedSource(env),
      ...
      fOutgoingPacketCounter(0),
      fProgramMapVersion(0xFF),
      fPreviousInputProgramMapVersion(0xFF),
      fCurrentInputProgramMapVersion(0),
      fPCR_PID(0),
      fCurrentPID(0),
      fInputBuffer(NULL),
      fInputBufferSize(0),
      ...
      fSegmentationDuration(0),
      fSegmentationIndication(1),
      fCurrentSegmentDuration(0.0),
      fPreviousPTS(0.0),
      fOnEndOfSegmentFunc(NULL),
      fOnEndOfSegmentClientData(NULL) {
...
Boolean MPEG2TransportStreamMultiplexor::isMPEG2TransportStreamMultiplexor()
        const {
    return True;
}
...
void MPEG2TransportStreamMultiplexor::doGetNextFrame() {
    ...
    do {
        ...
        if ((segmentationIsTimed() && fSegmentationIndication == 1) ||
            (!segmentationIsTimed() &&
             fOutgoingPacketCounter % PAT_PERIOD_IF_UNTIMED == 0)) {
        ...
        // Periodically (or when we see a new PID) return a Program Map Table
        // instead:
        Boolean programMapHasChanged = fCurrentInputProgramMapVersion !=
                                       fPreviousInputProgramMapVersion;
        if (programMapHasChanged ||
            (segmentationIsTimed() && fSegmentationIndication == 2) ||
            (!segmentationIsTimed() &&
             fOutgoingPacketCounter % PMT_PERIOD_IF_UNTIMED == 0)) {
    ...
    if ((fOutgoingPacketCounter % 10) == 0) {
        // To avoid excessive recursion (and stack overflow) caused by
        // excessively large input frames, occasionally return to the event loop
        // to do this:
...
void MPEG2TransportStreamMultiplexor ::handleNewBuffer(unsigned char* buffer,
    ...
    if (stream_id == 0xBE) {  // padding_stream; ignore
        ...
        if (streamType == 0) {
            ...
            // whether the stream is audio or video, and whether it's MPEG-1 or
            // MPEG-2:
            if ((stream_id & 0xF0) == 0xE0) {  // video
                ...
                streamType = mpegVersion == 1
                                     ? 3
                                     : mpegVersion == 2
                                               ? 4
                                               : mpegVersion == 3 ? 6 : 0xF;
            } else if (stream_id ==
                       0xBD) {      // private_stream1 (usually AC-3 or Opus)
        ...
        if (fPCR_PID == 0) {  // set it to this stream, if it's appropriate:
            if ((!fHaveVideoStreams &&
                 (streamType == 3 || streamType == 4 || streamType == 6 ||
...
void MPEG2TransportStreamMultiplexor ::deliverDataToClient(
    ...
    if (fMaxSize < TRANSPORT_PACKET_SIZE) {
        fFrameSize =
                0;  // the client hasn't given us enough space; deliver nothing
        ...
        Boolean willAddPCR = pid == fPCR_PID && startPositionInBuffer == 0 &&
                             !(fPCR.highBit == 0 && fPCR.remainingBits == 0 &&
                               fPCR.extension == 0);
        ...
        if (willAddPCR) {
            ...
            if (numBytesAvailable >=
                ...
                numPaddingBytes = TRANSPORT_PACKET_SIZE - numHeaderBytes -
                                  numPCRBytes - numDataBytes;
            }
        } else if (numBytesAvailable >=
                   TRANSPORT_PACKET_SIZE - numHeaderBytes) {
        ...
        *header++ = ((startPositionInBuffer == 0) ? 0x40 : 0x00) | (pid >> 8);
        ...
        // transport_priority, first 5 bits of PID
        *header++ = pid;
        // last 8 bits of PID
        ...
        if (adaptation_field_control == 0x30) {
            ...
            *header++ = adaptation_field_length;
            if (numHeaderBytes > 5) {
                ...
                *header++ = flags;
                if (willAddPCR) {
                    u_int32_t pcrHigh32Bits =
                            (fPCR.highBit << 31) | (fPCR.remainingBits >> 1);
                    ...
                    *header++ =
                            (u_int8_t)
                                    fPCR.extension;  // low 8 bits of extension

                    if (fSegmentationDuration > 0) {
                        // Use the PCR to compute the duration of the segment so
                        // far, to check whether segmentation needs to occur
                        // now:
...
void MPEG2TransportStreamMultiplexor::deliverPATPacket() {
    ...
    unsigned const patSize =
            TRANSPORT_PACKET_SIZE - 4;  // allow for the 4-byte header
    ...
    *pat++ = 0;     // pointer_field
    *pat++ = 0;     // table_id
    *pat++ = 0xB0;  // section_syntax_indicator; 0; reserved, section_length
                    // (high)
    *pat++ = 13;    // section_length (low)
    *pat++ = 0;
    *pat++ = 1;     // transport_stream_id
    *pat++ = 0xC1;  // reserved; version_number; current_next_indicator
    *pat++ = 0;     // section_number
    *pat++ = 0;     // last_section_number
    *pat++ = OUR_PROGRAM_NUMBER >> 8;
    *pat++ = OUR_PROGRAM_NUMBER;  // program_number
    *pat++ = 0xE0 |
             (OUR_PROGRAM_MAP_PID >> 8);  // reserved; program_map_PID (high)
    *pat++ = OUR_PROGRAM_MAP_PID & 0xFF;  // program_map_PID (low)

    // Compute the CRC from the bytes we currently have (not including
    // "pointer_field"):
...
void MPEG2TransportStreamMultiplexor::deliverPMTPacket(Boolean hasChanged) {
    ...
    *pmt++ = 0;     // pointer_field
    *pmt++ = 2;     // table_id
    *pmt++ = 0xB0;  // section_syntax_indicator; 0; reserved, section_length
                    // (high)
    ...
    *pmt++ = 0xC1 | ((fProgramMapVersion & 0x1F)
                     << 1);  // reserved; version_number; current_next_indicator
...
void MPEG2TransportStreamMultiplexor::setProgramStreamMap(unsigned frameSize) {
    if (frameSize <= 16)
        return;  // program_stream_map is too small to be useful
    if (frameSize > 0xFF) return;  // program_stream_map is too large

    u_int16_t program_stream_map_length =
            (fInputBuffer[4] << 8) | fInputBuffer[5];
    ...
    if ((versionByte & 0x80) == 0)
        return;  // "current_next_indicator" is not set
    fCurrentInputProgramMapVersion = versionByte & 0x1F;

    u_int16_t program_stream_info_length =
            (fInputBuffer[8] << 8) | fInputBuffer[9];
    unsigned offset =
            10 + program_stream_info_length;  // skip over 'descriptors'
...
u_int32_t calculateCRC(u_int8_t const* data,
                       unsigned dataLength,
