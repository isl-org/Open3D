...
Boolean MediaSession::isMediaSession() const { return True; }
...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
    ...
    while (sdpLine != NULL) {
        ...
        if ((sscanf(sdpLine, "m=%s %hu RTP/AVP %u", mediumName,
                    &subsession->fClientPortNum, &payloadFormat) == 3 ||
             sscanf(sdpLine, "m=%s %hu/%*u RTP/AVP %u", mediumName,
                    &subsession->fClientPortNum, &payloadFormat) == 3) &&
            payloadFormat <= 127) {
            protocolName = "RTP";
        } else if ((sscanf(sdpLine, "m=%s %hu UDP %u", mediumName,
                           &subsession->fClientPortNum, &payloadFormat) == 3 ||
                    sscanf(sdpLine, "m=%s %hu udp %u", mediumName,
                           &subsession->fClientPortNum, &payloadFormat) == 3 ||
        ...
        if (subsession->fRTPTimestampFrequency == 0) {
            subsession->fRTPTimestampFrequency = guessRTPTimestampFrequency(
...
Boolean MediaSession::parseSDPLine(char const* inputLine,
    ...
    if (strlen(inputLine) < 2 || inputLine[1] != '=' || inputLine[0] < 'a' ||
        inputLine[0] > 'z') {
...
Boolean MediaSession::parseSDPAttribute_type(char const* sdpLine) {
    // Check for a "a=type:broadcast|meeting|moderated|test|H.332|recvonly"
    // line:
    Boolean parseSuccess = False;
...
MediaSubsession::MediaSubsession(MediaSession& parent)
    ...
    setAttribute(
            "profile-level-id", "0",
            True /*value is hexadecimal*/);  // used with "video/H264"
                                             // This won't work for MPEG-4
                                             // (unless the value is <10),
                                             // because for MPEG-4, the value is
...
void MediaSubsession::addFilter(FramedFilter* filter) { fReadSource = filter; }
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
    ...
    do {
        ...
        if (fClientPortNum != 0 &&
            (honorSDPPortChoice || IsMulticastAddress(tempAddr.s_addr))) {
            ...
            if (protocolIsRTP && !fMultiplexRTCPWithRTP) {
                ...
                // use an even-numbered port for RTP, and the next
                // (odd-numbered) port for RTCP
            }
            if (isSSM()) {
                ...
                fRTPSocket =
                        new Groupsock(env(), tempAddr, fClientPortNum, 255);
            }
            if (fRTPSocket == NULL) {
            ...
            // Port numbers were not specified in advance, so we use ephemeral
            // port numbers. Create sockets until we get a port-number pair
            // (even: RTP; even+1: RTCP). (However, if we're multiplexing RTCP
            // with RTP, then we create only one socket, and the port number can
            // be even or odd.) We need to make sure that we don't keep trying
            // to use the same bad port numbers over and over again, so we store
            // bad sockets in a table, and delete them all when we're done.
            HashTable* socketHashTable = HashTable::create(ONE_WORD_HASH_KEYS);
            if (socketHashTable == NULL) break;
            Boolean success = False;
            NoReuse dummy(env());
            // ensures that our new ephemeral port number won't be one that's
            // already in use
            ...
            while (1) {
                ...
                if (fRTPSocket == NULL) {
                    env().setResultMsg(
                            "MediaSession::initiate(): unable to create RTP "
                            "and RTCP sockets");
                    break;
                }
                ...
                if ((fClientPortNum & 1) != 0) {  // it's odd
                    ...
                    Groupsock* existing = (Groupsock*)socketHashTable->Add(
                            reinterpret_cast<char const*>(key), fRTPSocket);
                    delete existing;  // in case it wasn't NULL
                    continue;
                }

                // Make sure we can use the next (i.e., odd) port number, for
                // RTCP:
                ...
                if (isSSM()) {
                    ...
                    fRTCPSocket =
                            new Groupsock(env(), tempAddr, rtcpPortNum, 255);
                }
                if (fRTCPSocket != NULL && fRTCPSocket->socketNum() >= 0) {
                    ...
                    // We couldn't create the RTCP socket (perhaps that port
                    // number's already in use elsewhere?).
                    delete fRTCPSocket;
                    fRTCPSocket = NULL;
            ...
            Groupsock* oldGS;
            while ((oldGS = (Groupsock*)socketHashTable->RemoveNext()) !=
                   NULL) {
                delete oldGS;
            }
            ...
            if (!success)
                break;  // a fatal error occurred trying to create the RTP and
                        // RTCP sockets; we can't continue
        }
        ...
        if (isSSM() && fRTCPSocket != NULL) {
            // Special case for RTCP SSM: Send RTCP packets back to the source
            // via unicast:
        ...
        if (fRTPSource != NULL && fRTCPSocket != NULL) {
            ...
            fRTCPInstance = RTCPInstance::createNew(
                    env(), fRTCPSocket, totSessionBandwidth,
                    (unsigned char const*)fParent.CNAME(),
...
double MediaSubsession::getNormalPlayTime(
    ...
    // First, check whether our "RTPSource" object has already been synchronized
    // using RTCP. If it hasn't, then - as a special case - we need to use the
    // RTP timestamp to compute the NPT.
    if (!rtpSource()->hasBeenSynchronizedUsingRTCP()) {
        if (!rtpInfo.infoIsNew)
            return 0.0;  // the "rtpInfo" structure has not been filled in
        u_int32_t timestampOffset =
                rtpSource()->curPacketRTPTimestamp() - rtpInfo.timestamp;
        double nptOffset = (timestampOffset /
                            (double)(rtpSource()->timestampFrequency())) *
                           scale();
        double npt = playStartTime() + nptOffset;

        return npt;
    } else {
        // Common case: We have been synchronized using RTCP.  This means that
        // the "presentationTime" parameter will be accurate, and so we should
        // use this to compute the NPT.
        double ptsDouble = (double)(presentationTime.tv_sec +
                                    presentationTime.tv_usec / 1000000.0);

        if (rtpInfo.infoIsNew) {
            // This is the first time we've been called with a synchronized
            // presentation time since the "rtpInfo" structure was last filled
            // in.  Use this "presentationTime" to compute "fNPT_PTS_Offset":
            ...
            // Use the precomputed "fNPT_PTS_Offset" to compute the NPT from the
            // PTS:
            if (fNPT_PTS_Offset == 0.0)
                return 0.0;  // error: The "rtpInfo" structure was apparently
                             // never filled in
...
void MediaSubsession ::setAttribute(char const* name,
    ...
    // Replace any existing attribute record with this name (except that the
    // 'valueIsHexadecimal' property will be inherited from it, if it exists).
...
Boolean MediaSubsession::parseSDPAttribute_rtpmap(char const* sdpLine) {
    ...
    if (sscanf(sdpLine, "a=rtpmap: %u %[^/]/%u/%u", &rtpmapPayloadFormat,
               codecName, &rtpTimestampFrequency, &numChannels) == 4 ||
        sscanf(sdpLine, "a=rtpmap: %u %[^/]/%u", &rtpmapPayloadFormat,
               codecName, &rtpTimestampFrequency) == 3 ||
        sscanf(sdpLine, "a=rtpmap: %u %s", &rtpmapPayloadFormat, codecName) ==
                2) {
        parseSuccess = True;
    ...
    do {
        ...
        if (strcmp(fProtocolName, "UDP") == 0) {
            ...
            if (strcmp(fCodecName, "MP2T") == 0) {  // MPEG-2 Transport Stream
                fReadSource = MPEG2TransportStreamFramer::createNew(
                        env(), fReadSource);
                // this sets "durationInMicroseconds" correctly, based on the
                // PCR values
            }
        } else {
            ...
            // (Also, add more fmts that can be implemented by
            // SimpleRTPSource#####)
            Boolean createSimpleRTPSource =
                    False;  // by default; can be changed below
            Boolean doNormalMBitRule =
                    False;  // default behavior if "createSimpleRTPSource" is
                            // True
            if (strcmp(fCodecName, "QCELP") == 0) {  // QCELP audio
                fReadSource = QCELPAudioRTPSource::createNew(
                        env(), fRTPSocket, fRTPSource, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
                // Note that fReadSource will differ from fRTPSource in this
                // case
            } else if (strcmp(fCodecName, "AMR") ==
                       0) {  // AMR audio (narrowband)
                fReadSource = AMRAudioRTPSource::createNew(
                        env(), fRTPSocket, fRTPSource, fRTPPayloadFormat,
                        False /*isWideband*/, fNumChannels,
                        attrVal_bool("octet-align"),
                        attrVal_unsigned("interleaving"),
                        attrVal_bool("robust-sorting"), attrVal_bool("crc"));
                // Note that fReadSource will differ from fRTPSource in this
                // case
            } else if (strcmp(fCodecName, "AMR-WB") ==
                       0) {  // AMR audio (wideband)
                fReadSource = AMRAudioRTPSource::createNew(
                        env(), fRTPSocket, fRTPSource, fRTPPayloadFormat,
                        True /*isWideband*/, fNumChannels,
                        attrVal_bool("octet-align"),
                        attrVal_unsigned("interleaving"),
                        attrVal_bool("robust-sorting"), attrVal_bool("crc"));
                // Note that fReadSource will differ from fRTPSource in this
                ...
                fReadSource = fRTPSource = MPEG1or2AudioRTPSource::createNew(
                        ...
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "MPA-ROBUST") ==
                       0) {  // robust MP3 audio
                fReadSource = fRTPSource = MP3ADURTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
                if (fRTPSource == NULL) break;

                if (!fReceiveRawMP3ADUs) {
                    // Add a filter that deinterleaves the ADUs after
                    // depacketizing them:
                    MP3ADUdeinterleaver* deinterleaver =
                            MP3ADUdeinterleaver::createNew(env(), fRTPSource);
                    if (deinterleaver == NULL) break;

                    // Add another filter that converts these ADUs to MP3
                    // frames:
                    fReadSource =
                            MP3FromADUSource::createNew(env(), deinterleaver);
                }
                ...
                fRTPSource = SimpleRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency, "audio/MPA-ROBUST" /*hack*/);
                if (fRTPSource == NULL) break;

                // Add a filter that converts these ADUs to MP3 frames:
                fReadSource = MP3FromADUSource::createNew(
                        env(), fRTPSource, False /*no ADU header*/);
            } else if (strcmp(fCodecName, "MP4A-LATM") ==
                       0) {  // MPEG-4 LATM audio
                fReadSource = fRTPSource = MPEG4LATMAudioRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "VORBIS") == 0) {  // Vorbis audio
                fReadSource = fRTPSource = VorbisAudioRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "THEORA") == 0) {  // Theora video
                fReadSource = fRTPSource = TheoraVideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat);
            } else if (strcmp(fCodecName, "RAW") ==
                       0) {  // Uncompressed raw video (RFC 4175)
                fReadSource = fRTPSource = RawVideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "VP8") == 0) {  // VP8 video
                fReadSource = fRTPSource = VP8VideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "VP9") == 0) {  // VP9 video
                fReadSource = fRTPSource = VP9VideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
                ...
                fReadSource = fRTPSource = AC3AudioRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "MP4V-ES") ==
                       0) {  // MPEG-4 Elementary Stream video
                ...
                fReadSource = fRTPSource = MPEG4GenericRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency, fMediumName,
                        attrVal_strToLower("mode"),
                ...
                fReadSource = fRTPSource = MPEG1or2VideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "MP2T") ==
                       0) {  // MPEG-2 Transport Stream
                fRTPSource = SimpleRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency, "video/MP2T", 0, False);
                fReadSource = MPEG2TransportStreamFramer::createNew(env(),
                                                                    fRTPSource);
                // this sets "durationInMicroseconds" correctly, based on the
                // PCR values
            } else if (strcmp(fCodecName, "H261") == 0) {  // H.261
                fReadSource = fRTPSource = H261VideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
                ...
                fReadSource = fRTPSource = H265VideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat, expectDONFields,
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "DV") == 0) {
                fReadSource = fRTPSource = DVVideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency);
            } else if (strcmp(fCodecName, "JPEG") == 0) {  // motion JPEG
                if (fReceiveRawJPEGFrames) {
                    // Special case (used when proxying JPEG/RTP streams):
                    // Receive each JPEG/RTP packet, including the special RTP
                    // headers:
                    fReadSource = fRTPSource = SimpleRTPSource::createNew(
                            env(), fRTPSocket, fRTPPayloadFormat,
                            fRTPTimestampFrequency, "video/JPEG",
                            0 /*special offset*/,
                            False /*doNormalMBitRule => ignore the 'M' bit*/);
                } else {
                    // Normal case: Receive each JPEG frame as a complete,
                    // displayable JPEG image:
                    fReadSource = fRTPSource = JPEGVideoRTPSource::createNew(
                            env(), fRTPSocket, fRTPPayloadFormat,
                            fRTPTimestampFrequency, videoWidth(),
                            videoHeight());
                }
            } else if (strcmp(fCodecName, "JPEG2000") ==
                       0) {  // JPEG 2000 video
                fReadSource = fRTPSource = JPEG2000VideoRTPSource::createNew(
                        env(), fRTPSocket, fRTPPayloadFormat,
                        fRTPTimestampFrequency, attrVal_str("sampling"));
                ...
                delete[] mimeType;
                       ...
                       || strcmp(fCodecName, "DVI4") ==
                                  0  // DVI4 (IMA ADPCM) audio
                       ...
                       || strcmp(fCodecName, "L20") ==
                                  0  // 20-bit linear audio (RFC 3190)
                       || strcmp(fCodecName, "L24") ==
                                  0  // 24-bit linear audio (RFC 3190)
                       || strcmp(fCodecName, "G722") ==
                                  0  // G.722 audio (RFC 3551)
                       ...
                       || strcmp(fCodecName, "DAT12") ==
                                  0  // 12-bit nonlinear audio (RFC 3190)
                ...
                if (strcmp(fCodecName, "VND.ONVIF.METADATA") == 0) {
                    // This RTP payload format uses the RTP "M" bit to indicate
                    // the end of the content (a XML document):
                    doNormalMBitRule = True;
                }
            } else if (useSpecialRTPoffset >= 0) {
                // We don't know this RTP payload format, but try to receive
                // it using a 'SimpleRTPSource' with the specified header
                // offset:
                createSimpleRTPSource = True;
            } else {
                env().setResultMsg(
...
SDPAttribute::SDPAttribute(char const* strValue, Boolean valueIsHexadecimal)
    : fStrValue(strDup(strValue)),
      fStrValueToLower(NULL),
      fValueIsHexadecimal(valueIsHexadecimal) {
    if (fStrValue == NULL) {
        // No value was given for this attribute, so consider it to be a
        ...
        // Try to parse "fStrValueToLower" as an integer.  If we can't, assume
        // an integer value of 0:
        if (sscanf(fStrValueToLower, valueIsHexadecimal ? "%x" : "%d",
                   &fIntValue) != 1) {
            fIntValue = 0;
        }
    }
