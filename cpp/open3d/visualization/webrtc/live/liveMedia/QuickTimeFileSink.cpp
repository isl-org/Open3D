...
#define H264_IDR_FRAME \
    0x65  // bit 8 == 0, bits 7-6 (ref) == 3, bits 5-0 (type) == 5
...
public:
    SubsessionBuffer(unsigned bufferSize) : fBufferSize(bufferSize) {
        reset();
...
private:
    ...
    struct {
        ...
        unsigned rtpHeader;
        unsigned char numSpecialHeaders;    // used when our RTP source has
                                            // special headers
...
QuickTimeFileSink::QuickTimeFileSink(UsageEnvironment& env,
    ...
    while ((subsession = iter.next()) != NULL) {
        ...
        SubsessionIOState* ioState = new SubsessionIOState(*this, *subsession);
...
QuickTimeFileSink* QuickTimeFileSink::createNew(UsageEnvironment& env,
                                                ...
                                                Boolean generateHintTracks,
                                                Boolean generateMP4Format) {
    QuickTimeFileSink* newSink = new QuickTimeFileSink(
            env, inputSession, outputFileName, bufferSize, movieWidth,
            movieHeight, movieFPS, packetLossCompensate, syncStreams,
...
void QuickTimeFileSink ::noteRecordedFrame(
        MediaSubsession& /*inputSubsession*/,
...
void QuickTimeFileSink ::afterGettingFrame(
        void* clientData,
        unsigned packetDataSize,
    ...
    if (numTruncatedBytes > 0) {
        ioState->envir() << "QuickTimeFileSink::afterGettingFrame(): The input "
                            "frame data was too large for our buffer.  "
                         << numTruncatedBytes
                         << " bytes of trailing data was dropped!  Correct "
                            "this by increasing the \"bufferSize\" parameter "
                            "in the \"createNew()\" call.\n";
    }
...
void QuickTimeFileSink::onRTCPBye(void* clientData) {
    ...
    ioState->envir() << "Received RTCP \"BYE\" on \"" << subsession.mediumName()
...
SubsessionIOState::SubsessionIOState(QuickTimeFileSink& sink,
    ...
    fPrevBuffer = sink.fPacketLossCompensate
                          ? new SubsessionBuffer(fOurSink.fBufferSize)
                          : NULL;
...
Boolean SubsessionIOState::setQTstate() {
    ...
    char const* noCodecWarning3 =
            "\" track, so we'll insert a dummy \"????\" Media Data Atom "
    ...
    do {
        ...
        if (isHintTrack()) {
            ...
            fQTMediaDataAtomCreator =
                    &QuickTimeFileSink::addAtom_soundMediaGeneral;  // by
                                                                    // default
            ...
            if (strcmp(fOurSubsession.codecName(), "X-QT") == 0 ||
                ...
                fQTMediaDataAtomCreator =
                        &QuickTimeFileSink::addAtom_genericMedia;
                ...
                fQTSamplesPerFrame = 160;
            } else if (strcmp(fOurSubsession.codecName(), "MPEG4-GENERIC") ==
                               0 ||
                       strcmp(fOurSubsession.codecName(), "MP4A-LATM") == 0) {
                fQTMediaDataAtomCreator = &QuickTimeFileSink::addAtom_mp4a;
                fQTTimeUnitsPerSample =
                        1024;  // QT considers each frame to be a 'sample'
                // The time scale (frequency) comes from the 'config'
                // information. It might be different from the RTP timestamp
                // frequency (e.g., aacPlus).
            ...
            envir() << "Warning: We don't implement a QuickTime Media Handler "
                       "for media type \""
        ...
        envir() << "Warning: We don't have sufficient codec-specific "
                   "information (e.g., sample sizes) to fully generate the \""
                << fOurSubsession.mediumName() << "/"
                << fOurSubsession.codecName()
                << "\" track, so we'll disable this track in the movie.  A "
                   "separate, codec-specific editing pass will be needed "
                   "before this track can be played\n";
        fQTEnableTrack = False;  // disable this track in the movie
#endif
    ...
    envir() << ", so a track for the \"" << fOurSubsession.mediumName() << "/"
            << fOurSubsession.codecName()
            << "\" subsession will not be included in the output QuickTime "
               "file\n";
    return False;
}
...
void SubsessionIOState::afterGettingFrame(unsigned packetDataSize,
    ...
    if (fOurSubsession.rtpSource() != NULL) {  // we have a RTP stream
        ...
        if (fOurSink.fPacketLossCompensate && fPrevBuffer->bytesInUse() > 0) {
            ...
            for (short i = 1; i < seqNumGap; ++i) {
                // Insert a copy of the previous frame, to compensate for the
                // loss:
    ...
    fOurSink.noteRecordedFrame(fOurSubsession, packetDataSize,
                               presentationTime);
...
void SubsessionIOState::useFrame(SubsessionBuffer& buffer) {
    ...
    if (!fOurSink.fSyncStreams ||
        fQTcomponentSubtype != fourChar('v', 'i', 'd', 'e')) {
        unsigned const frameDuration =
                fQTTimeUnitsPerSample * fQTSamplesPerFrame;
        ...
        if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
            ...
            double duration =
                    (presentationTime.tv_sec - ppt.tv_sec) +
            ...
            unsigned numSamples = useFrame1(frameSizeToUse, ppt, frameDuration,
    ...
    // If we have a hint track, then write to it also (only if we have a RTP
    // stream):
...
void SubsessionIOState::useFrameForHinting(unsigned frameSize,
    ...
    // For the hint track, we need to split the frame back up into separate
    // packets. However, for some RTP sources, then we also need to reuse the
    // special header bytes that were at the start of each of the RTP packets.
    Boolean hack263 = strcmp(fOurSubsession.codecName(), "H263-1998") == 0;
    Boolean hackm4a_generic =
            strcmp(fOurSubsession.mediumName(), "audio") == 0 &&
    ...
    RTPSource* const rs =
            fOurSubsession.rtpSource();  // abbrev (ASSERT: != NULL)
    struct timeval const& ppt = fPrevFrameState.presentationTime;  // abbrev
    if (ppt.tv_sec != 0 || ppt.tv_usec != 0) {
        ...
        unsigned hintSampleDuration =
                (unsigned)((2 * duration * fQTTimeScale + 1) / 2);  // round
        if (hackm4a) {
            // Because multiple AAC frames can appear in a RTP packet, the
            // presentation times of the second and subsequent frames will not
            // be accurate. So, use the known "hintSampleDuration" instead:
            hintSampleDuration = fTrackHintedByUs->fQTTimeUnitsPerSample;

            // Also, if the 'time scale' was different from the RTP timestamp
            // frequency, (as can happen with aacPlus), then we need to scale
            // "hintSampleDuration" accordingly:
            if (fTrackHintedByUs->fQTTimeScale !=
                fOurSubsession.rtpTimestampFrequency()) {
                unsigned const scalingFactor =
                        fOurSubsession.rtpTimestampFrequency() /
                        fTrackHintedByUs->fQTTimeScale;
        ...
        unsigned short numPTEntries =
                (fPrevFrameState.frameSize + (maxPacketSize - 1)) /
                maxPacketSize;  // normal case
        ...
        for (unsigned i = 0; i < numPTEntries; ++i) {
            ...
            if (haveSpecialHeaders) {  // special case
                ...
                if (hack263) {
                    Boolean PbitSet = immediateDataLen >= 1 &&
                                      (immediateDataPtr[0] & 0x4) != 0;
                    if (PbitSet) {
                        offsetWithinSample +=
                                2;  // to omit the two leading 0 bytes
                    }
                }
            }
            ...
            hintSampleSize +=
                    fOurSink.addWord(0);  // Relative transmission time
            ...
            hintSampleSize +=
                    fOurSink.addHalfWord(numDTEntries);  // Entry count
            ...
            if (haveSpecialHeaders) {
                ...
                for (j = 0; j < len; ++j) {
                    hintSampleSize +=
                            fOurSink.addByte(immediateDataPtr[j]);  // Data
                }
                for (j = len; j < 14; ++j) {
            ...
            offsetWithinSample +=
                    dataFrameSize;  // for the next iteration (if any)
    ...
    fPrevFrameState.rtpHeader = rs->curPacketMarkerBit() << 23 |
    ...
    if (hack263) {
        ...
        // Synthesize a special header, so that this frame can be in its own RTP
        // packet.
        unsigned const sizeLength =
                fOurSubsession.attrVal_unsigned("sizelength");
        unsigned const indexLength =
                fOurSubsession.attrVal_unsigned("indexlength");
        ...
        fPrevFrameState.specialHeaderBytes[0] =
                0;  // AU_headers_length (high byte)
        fPrevFrameState.specialHeaderBytes[1] =
                16;  // AU_headers_length (low byte)
        fPrevFrameState.specialHeaderBytes[2] =
                ((frameSize << indexLength) & 0xFF00) >> 8;
...
unsigned SubsessionIOState::useFrame1(unsigned sourceDataSize,
    ...
    if (fTailChunk == NULL) {
        newTailChunk = fHeadChunk =
                new ChunkDescriptor(destFileOffset, sourceDataSize, frameSize,
                                    frameDuration, presentationTime);
    } else {
    ...
    if (s.fNumSyncedSubsessions < s.fNumSubsessions) {
        ...
        if (!fHaveBeenSynced) {
            ...
            if (fOurSubsession.rtpSource()->hasBeenSynchronizedUsingRTCP()) {
                // H264 ?
                if (fQTMediaDataAtomCreator ==
                    &QuickTimeFileSink::addAtom_avc1) {
                    // special case: audio + H264 video: wait until audio is in
                    // sync
...
ChunkDescriptor* ChunkDescriptor ::extendChunk(
        int64_t newOffsetInFile,
        unsigned newSize,
        unsigned newFrameSize,
        unsigned newFrameDuration,
        struct timeval newPresentationTime) {
    ...
    ChunkDescriptor* newDescriptor =
            new ChunkDescriptor(newOffsetInFile, newSize, newFrameSize,
                                newFrameDuration, newPresentationTime);
...
unsigned QuickTimeFileSink::addArbitraryString(char const* str,
    ...
    if (oneByteLength) {
        ...
        if (strLength >= 256) {
            envir() << "QuickTimeFileSink::addArbitraryString(\"" << str
...
void QuickTimeFileSink::setWord(int64_t filePosn, unsigned size) {
    ...
    // One of the SeekFile64()s failed, probable because we're not a seekable
    // file
...
unsigned QuickTimeFileSink::addAtom_hdlr2() {
    ...
    unsigned QuickTimeFileSink::addAtom_genericMedia() {
        ...
        unsigned QuickTimeFileSink::addAtom_soundMediaGeneral() {
            ...
            unsigned short const version =
                    fCurrentIOState->fQTSoundSampleVersion;
            size += addWord(version << 16);  // Version+Revision level
            size += addWord(0x00000000);     // Vendor
            unsigned short numChannels =
                    (unsigned short)(fCurrentIOState->fOurSubsession
                                             .numChannels());
            ...
            unsigned const sampleRateFixedPoint = fCurrentIOState->fQTTimeScale
                                                  << 16;
            size += addWord(sampleRateFixedPoint);  // Sample rate
            addAtomEnd;

            unsigned QuickTimeFileSink::addAtom_Qclp() {
                // The beginning of this atom looks just like a general Sound
                // Media atom, except with a version field of 1:
                int64_t initFilePosn = TellFile64(fOutFid);
                ...
                if (strcmp(fCurrentIOState->fQTAudioDataType, "Qclp") == 0) {
                    ...
                    size += addWord(0x00000000);  // ???
                    size += addWord(0x00000008);  // ???
                } else if (strcmp(fCurrentIOState->fQTAudioDataType, "mp4a") ==
                           0) {
                ...
                addAtom(frma);
                size += add4ByteString(
                        fCurrentIOState->fQTAudioDataType);  // ???
                addAtomEnd;
                ...
                unsigned QuickTimeFileSink::addAtom_mp4a() {
                    unsigned size = 0;
                    // The beginning of this atom looks just like a general
                    // Sound Media atom, except with a version field of 1:
                    int64_t initFilePosn = TellFile64(fOutFid);
                    ...
                    if (fGenerateMP4Format) {
                        ...
                        // Next, add the four fields that are particular to
                        // version 1: (Later, parameterize these #####)
                        size += addWord(fCurrentIOState->fQTTimeUnitsPerSample);
                        size += addWord(0x00000001);  // ???
                        size += addWord(0x00000001);  // ???
                        size += addWord(
                                0x00000002);  // bytes per sample (uncompressed)

                        // Other special fields are in a 'wave' atom that
                        // follows:
                        size += addAtom_wave();
                    }
                    addAtomEnd;
                    ...
                    //#####
                    MediaSubsession& subsession =
                            fCurrentIOState->fOurSubsession;
                    ...
                    unsigned configSize;
                    unsigned char* config = parseGeneralConfigStr(
                            subsession.fmtp_config(), configSize);
                    ...
                    size += addWord(0x00000000);  // Reserved
                    size += addWord(
                            0x00000001);  // Reserved+Data reference index
                    ...
                    unsigned const widthAndHeight =
                            (fMovieWidth << 16) | fMovieHeight;
                    ...
                    size += addWord(0x00010548);  // Frame count+Compressor name
                                                  // (start) "H.263"
                    size += addWord(0x2e323633);  // Compressor name (continued)
                    size += addZeroWords(
                            6);  // Compressor name (continued - zero)
                    size += addWord(
                            0x00000018);  // Compressor name (final)+Depth
                    ...
                    addAtom(avcC);
                    // Begin by Base-64 decoding the "sprop" parameter sets
                    // strings:
                    char* psets = strDup(fCurrentIOState->fOurSubsession
                    ...
                    unsigned char* sps_data =
                            base64Decode(sps_b64, sps_count, false);
                    unsigned pps_count;
                    unsigned char* pps_data =
                            base64Decode(pps_b64, pps_count, false);
                    ...
                    size += addWord(0x00480000);      // Vertical resolution
                    size += addWord(0x00000000);      // Data size
                    size += addWord(0x00010c4d);  // Frame count+Compressor name
                                                  // (start) "MPEG-4 Video"
                    ...
                    size += addZeroWords(
                            4);  // Compressor name (continued - zero)
                    size += addWord(
                            0x00000018);  // Compressor name (final)+Depth
                    size += addHalfWord(0xffff);  // Color table id
                    size += addAtom_esds();       // ESDescriptor
                    ...
                    unsigned QuickTimeFileSink::addAtom_rtp() {
                        ...
                        size += addWord(0x00000001);  // Reserved (last 2 bytes)
                                                      // + Data ref index
                        size += addWord(0x00010001);  // Hint track version +
                                                      // Last compat htv
                        ...
                        addAtom(tims);
                        size += addWord(fCurrentIOState->fOurSubsession
                        ...
                        // (and remember its position).  We'll fill this field
                        // in later:
                        int64_t numEntriesPosition = TellFile64(fOutFid);
                        size += addWord(0);  // dummy for "Number of entries"

                        // Then, run through the chunk descriptors, and enter
                        // the entries in this (compressed) Time-to-Sample
                        // table:
                        unsigned numEntries = 0, numSamplesSoFar = 0;
                        unsigned prevSampleDuration = 0;
                        unsigned const samplesPerFrame =
                                fCurrentIOState->fQTSamplesPerFrame;
                        ChunkDescriptor* chunk = fCurrentIOState->fHeadChunk;
                        while (chunk != NULL) {
                            unsigned const sampleDuration =
                                    chunk->fFrameDuration / samplesPerFrame;
                            if (sampleDuration != prevSampleDuration) {
                                ...
                                if (chunk != fCurrentIOState->fHeadChunk) {
                                    ++numEntries;
                                    size += addWord(
                                            numSamplesSoFar);  // Sample count
                                    size += addWord(
                                            prevSampleDuration);  // Sample
                                                                  // duration
                                    numSamplesSoFar = 0;
                                }
                            }
                            ...
                            unsigned const numSamples =
                                    chunk->fNumFrames * samplesPerFrame;
                        ...
                        // Now go back and fill in the "Number of entries"
                        // field:
                        ...
                        if (fCurrentIOState->fHeadSyncFrame != NULL) {
                            SyncFrame* currentSyncFrame =
                                    fCurrentIOState->fHeadSyncFrame;

                            // First, count the number of frames (to use as a
                            // sanity check; see below):
                            unsigned totNumFrames = 0;
                            for (ChunkDescriptor* chunk =
                                         fCurrentIOState->fHeadChunk;
                                 chunk != NULL; chunk = chunk->fNextChunk)
                                totNumFrames += chunk->fNumFrames;

                            while (currentSyncFrame != NULL) {
                                if (currentSyncFrame->sfFrameNum >=
                                    totNumFrames)
                                    break;  // sanity check
                                ...
                                currentSyncFrame =
                                        currentSyncFrame->nextSyncFrame;
                            }
                        } else {
                            // First, run through the chunk descriptors,
                            // counting up the total number of samples:
                            unsigned const samplesPerFrame =
                                    fCurrentIOState->fQTSamplesPerFrame;
                            ...
                            // Then, write out the sample numbers that we deem
                            // correspond to 'sync samples':
                            unsigned i;
                            for (i = 0; i < numSamplesSoFar; i += 12) {
                                ...
                                // (Perhaps we should really try to keep track
                                // of which 'samples' ('frames' for video)
                                // really are 'key frames'?)
                                size += addWord(i + 1);
                                ++numEntries;
                            }

                            // Then, write out the last entry (if we haven't
                            // already done so):
                        ...
                        // Then, run through the chunk descriptors, and enter
                        // the entries in this (compressed) Sample-to-Chunk
                        // table:
                        ...
                        while (chunk != NULL) {
                            ...
                            if (samplesPerChunk != prevSamplesPerChunk) {
                                ...
                                size += addWord(
                                        samplesPerChunk);  // Samples per chunk
                                size += addWord(
                                        0x00000001);  // Sample description ID
                        ...
                        // Begin by checking whether our chunks all have the
                        // same 'bytes-per-sample'.  This determines whether
                        // this atom's table has just a single entry, or
                        // multiple entries.
                        Boolean haveSingleEntryTable = True;
                        double firstBPS = 0.0;
                        ChunkDescriptor* chunk = fCurrentIOState->fHeadChunk;
                        while (chunk != NULL) {
                            double bps = (double)(chunk->fFrameSize) /
                                         (fCurrentIOState->fQTSamplesPerFrame);
                            if (bps < 1.0) {
                                // I don't think a multiple-entry table would
                                // make sense in this case, so assume a single
                                // entry table ??? #####
                                break;
                            }
                        ...
                        if (haveSingleEntryTable) {
                            if (fCurrentIOState->isHintTrack() &&
                                ...
                                // The following doesn't seem right, but seems
                        ...
                        unsigned const totNumSamples =
                                fCurrentIOState->fQTTotNumSamples;
                        size += addWord(totNumSamples);  // Number of entries

                        if (!haveSingleEntryTable) {
                            // Multiple-entry table:
                            // Run through the chunk descriptors, entering the
                            // sample sizes:
                            ChunkDescriptor* chunk =
                                    fCurrentIOState->fHeadChunk;
                            while (chunk != NULL) {
                        ...
                        size += addWord(
                                fCurrentIOState
                                        ->fNumChunks);  // Number of entries

                        // Run through the chunk descriptors, entering the file
                        // offsets:
                        ChunkDescriptor* chunk = fCurrentIOState->fHeadChunk;
                        while (chunk != NULL) {
                        ...
                        size += addArbitraryString(description,
                                                   False);  // name of object
                        addAtomEnd;
                        ...
                        unsigned QuickTimeFileSink::addAtom_sdp() {
                            ...
                            char const* sdpLines =
                                    fCurrentIOState->fOurSubsession
                                            .savedSDPLines();
                            // We need to change any "a=control:trackID=" values
                            // to be this track's actual track id:
                            char* newSDPLines = new char[strlen(sdpLines) +
                                                         100 /*overkill*/];
                            char const* searchStr = "a=control:trackid=";
                            Boolean foundSearchString = False;
                            char const *p1, *p2, *p3;
                            for (p1 = sdpLines; *p1 != '\0'; ++p1) {
                                for (p2 = p1, p3 = searchStr;
                                     tolower(*p2) == *p3; ++p2, ++p3) {
                                }
                                if (*p3 == '\0') {
                                    // We found the end of the search string, at
                                    // p2.
                                    int beforeTrackNumPosn = p2 - sdpLines;
                                    // Look for the subsequent track number, and
                                    // skip over it:
                                    int trackNumLength;
                                    if (sscanf(p2, " %*d%n", &trackNumLength) <
                                        0)
                                        break;
                                    int afterTrackNumPosn =
                                            beforeTrackNumPosn + trackNumLength;

                                    // Replace the old track number with the
                                    // correct one:
                                    int i;
                                    for (i = 0; i < beforeTrackNumPosn; ++i)
                                        newSDPLines[i] = sdpLines[i];
                                    sprintf(&newSDPLines[i], "%d",
                                            fCurrentIOState->fTrackID);
                                    i = afterTrackNumPosn;
                                    int j = i + strlen(&newSDPLines[i]);
                                    while (1) {
                                        if ((newSDPLines[j] = sdpLines[i]) ==
                                            '\0')
                                            break;
                                        ++i;
                                        ++j;
                            ...
                            if (!foundSearchString) {
                                ...
                                // "a=control:trackID=<trackId>" line, add one
                                // of our own:
                                sprintf(newSDPLines, "%s%s%d\r\n", sdpLines,
                                        searchStr, fCurrentIOState->fTrackID);
                            }
                            ...
                            if (rtpSource != NULL) {
                                ...
                                // <mime-subtype>/<rtp-frequency>
                                unsigned rtpmapStringLength =
                                        strlen(ourSubsession.codecName()) + 20;
                                char* rtpmapString =
                                        new char[rtpmapStringLength];
                                sprintf(rtpmapString, "%s/%d",
                                        ourSubsession.codecName(),
