...
JPEGVideoRTPSource* JPEGVideoRTPSource::createNew(
    ...
    return new JPEGVideoRTPSource(env, RTPgs, rtpPayloadFormat,
                                  rtpTimestampFrequency, defaultWidth,
                                  defaultHeight);
}
...
Boolean JPEGVideoRTPSource ::processSpecialHeader(
  ...
  +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    */
    ...
    unsigned Offset =
            (unsigned)((DWORD)headerStart[1] << 16 |
                       (DWORD)headerStart[2] << 8 | (DWORD)headerStart[3]);
    ...
    if ((width == 0 || height == 0) && fDefaultWidth != 0 &&
        fDefaultHeight != 0) {
    ...
    if (Offset == 0) {
        if (Q > 127) {
            ...
            */
            ...
            if (MBZ == 0) {
                // unsigned Precision =
                // (unsigned)headerStart[resultSpecialHeaderSize + 1];
                unsigned Length =
                        (unsigned)((WORD)headerStart[resultSpecialHeaderSize +
                                                     2]
                                           << 8 |
                                   (WORD)headerStart[resultSpecialHeaderSize +
                                                     3]);
...
char const* JPEGVideoRTPSource::MIMEtype() const { return "video/JPEG"; }
...
unsigned JPEGBufferedPacket ::nextEnclosedFrameSize(unsigned char*& framePtr,
    ...
    if (completesFrame && dataSize >= 2 &&
        !(framePtr[dataSize - 2] == 0xFF &&
          framePtr[dataSize - 1] == MARKER_EOI)) {
...
BufferedPacket* JPEGBufferedPacketFactory ::createNewPacket(
