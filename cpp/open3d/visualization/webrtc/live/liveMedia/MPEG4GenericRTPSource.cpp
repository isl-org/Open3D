...
MPEG4GenericRTPSource* MPEG4GenericRTPSource::createNew(
    ...
    return new MPEG4GenericRTPSource(env, RTPgs, rtpPayloadFormat,
                                     rtpTimestampFrequency, mediumName, mode,
...
MPEG4GenericRTPSource ::MPEG4GenericRTPSource(UsageEnvironment& env,
    ...
    : MultiFramedRTPSource(env,
      ...
      fSizeLength(sizeLength),
      fIndexLength(indexLength),
    ...
    if (mode == NULL ||
        ...
        envir() << "MPEG4GenericRTPSource Warning: Unknown or unsupported "
                   "\"mode\": "
                << mode << "\n";
    }
}
...
Boolean MPEG4GenericRTPSource ::processSpecialHeader(
...
char const* MPEG4GenericRTPSource::MIMEtype() const { return fMIMEType; }
...
MPEG4GenericBufferedPacket ::MPEG4GenericBufferedPacket(
        MPEG4GenericRTPSource* ourSource)
...
BufferedPacket* MPEG4GenericBufferedPacketFactory ::createNewPacket(
...
unsigned samplingFrequencyFromAudioSpecificConfig(char const* configStr) {
    ...
    do {
        ...
        result = ((config[1] & 0x7F) << 17) | (config[2] << 9) |
                 (config[3] << 1) | (config[4] >> 7);
    } while (0);
