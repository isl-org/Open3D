...
Boolean MPEG1or2VideoRTPSink ::frameCanAppearAfterPacketStart(
    ...
    // A 'frame' (which in this context can mean a header or a slice as well as
    // a complete picture) can appear at other than the first position in a
    // packet in all situations, EXCEPT when it follows the end of (i.e., the
    // last slice of) a picture.  I.e., the headers at the beginning of a
    // picture must appear at the start of a RTP packet.
    ...
    return numBytesInFrame >= 4 && frameStart[0] == 0 && frameStart[1] == 0 &&
           frameStart[2] == 1 && frameStart[3] >= 1 && frameStart[3] <= 0xAF;
}
...
void MPEG1or2VideoRTPSink ::doSpecialFrameHandling(
    ...
    if (fragmentationOffset == 0) {
        ...
        if (startCode == VIDEO_SEQUENCE_HEADER_START_CODE) {
            ...
            unsigned next4Bytes = (frameStart[4] << 24) |
                                  (frameStart[5] << 16) | (frameStart[6] << 8) |
                                  frameStart[7];
            unsigned char byte8 = numBytesInFrame == 8 ? 0 : frameStart[8];

            fPictureState.temporal_reference =
                    (next4Bytes & 0xFFC00000) >> (32 - 10);
            fPictureState.picture_coding_type =
                    (next4Bytes & 0x00380000) >> (32 - (10 + 3));
            ...
            switch (fPictureState.picture_coding_type) {
                ...
                case 2:
                    ...
                    FFC = ((next4Bytes & 0x00000003) << 1) |
                          ((byte8 & 0x80) >> 7);
            }

            fPictureState.vector_code_bits =
                    (FBV << 7) | (BFC << 4) | (FFV << 3) | FFC;
            ...
            envir() << "Warning: MPEG1or2VideoRTPSink::doSpecialFrameHandling "
                       "saw "
                       "strange first 4 bytes "
                    << reinterpret_cast<void*>(startCode)
    ...
    unsigned videoSpecificHeader =
            ...
            // AN == N == 0
            (fSequenceHeaderPresent << 13) | (fPacketBeginsSlice << 12) |
            (fPacketEndsSlice << 11) |
    ...
    MPEG1or2VideoStreamFramer* framerSource =
            (MPEG1or2VideoStreamFramer*)fSource;
