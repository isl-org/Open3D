...
MPEG4VideoStreamDiscreteFramer* MPEG4VideoStreamDiscreteFramer::createNew(
...
void MPEG4VideoStreamDiscreteFramer ::afterGettingFrame(
        void* clientData,
        unsigned frameSize,
    ...
    MPEG4VideoStreamDiscreteFramer* source =
            (MPEG4VideoStreamDiscreteFramer*)clientData;
...
void MPEG4VideoStreamDiscreteFramer ::afterGettingFrame1(
    ...
    if (frameSize >= 4 && fTo[0] == 0 && fTo[1] == 0 && fTo[2] == 1) {
        ...
        if (fTo[i] == 0xB0) {  // VISUAL_OBJECT_SEQUENCE_START_CODE
            ...
            // This information (should) also contain a VOL header, which we
            // need to analyze, to get "vop_time_increment_resolution" (which we
            // need
        ...
        if (i < frameSize) {
            ...
            if (nextCode == 0xB6 /*VOP_START_CODE*/ && i + 5 < frameSize) {
                ...
                // Next, get the "modulo_time_base" by counting the '1' bits
                // that follow.  We look at the next 32-bits only. This should
                // be enough in most cases.
                u_int32_t next4Bytes = (fTo[i] << 24) | (fTo[i + 1] << 16) |
                                       (fTo[i + 2] << 8) | fTo[i + 3];
                i += 4;
                ...
                // If this is a "B" frame, then we have to tweak
                // "presentationTime":
                if (!fLeavePresentationTimesUnmodified &&
                    vop_coding_type == 2 /*B*/
                    && (fLastNonBFramePresentationTime.tv_usec > 0 ||
                        fLastNonBFramePresentationTime.tv_sec > 0)) {
                    int timeIncrement = fLastNonBFrameVop_time_increment -
                                        vop_time_increment;
                    if (timeIncrement < 0)
                    ...
                    unsigned secondsToSubtract =
                            (unsigned)(usIncrement / MILLION);
                    unsigned uSecondsToSubtract =
                            ((unsigned)usIncrement) % MILLION;

                    presentationTime = fLastNonBFramePresentationTime;
                    if ((unsigned)presentationTime.tv_usec <
                        uSecondsToSubtract) {
...
void MPEG4VideoStreamDiscreteFramer::analyzeVOLHeader() {
    ...
    for (i = 3; i < fNumConfigBytes; ++i) {
        if (fConfigBytes[i] >= 0x20 && fConfigBytes[i] <= 0x2F &&
            fConfigBytes[i - 1] == 1 && fConfigBytes[i - 2] == 0 &&
            fConfigBytes[i - 3] == 0) {
            ++i;
            break;
        }
