...
// A filter that converts a MPEG Transport Stream file - with corresponding
// index file
// - to a corresponding Video Elementary Stream.  It also uses a "scale"
// parameter to implement 'trick mode' (fast forward or reverse play, using
// I-frames) on the video stream. Implementation

#include "MPEG2TransportStreamTrickModeFilter.hh"

#include <ByteStreamFileSource.hh>

// Define the following to be True if we want the output file to have the same
// frame rate as the original file.
//    (Because the output file contains I-frames only, this means that each
//    I-frame will appear in the output file
//     several times, and therefore the output file's bitrate will be
//     significantly higher than that of the original.)
// Define the following to be False if we want the output file to include each
// I-frame no more than once.
//    (This means that - except for high 'scale' values - both the output frame
//    rate and the output bit rate
//     will be less than that of the original.)
#define KEEP_ORIGINAL_FRAME_RATE False

MPEG2TransportStreamTrickModeFilter*
MPEG2TransportStreamTrickModeFilter ::createNew(
        UsageEnvironment& env,
        FramedSource* inputSource,
        MPEG2TransportStreamIndexFile* indexFile,
        int scale) {
    return new MPEG2TransportStreamTrickModeFilter(env, inputSource, indexFile,
                                                   scale);
}
...
MPEG2TransportStreamTrickModeFilter ::MPEG2TransportStreamTrickModeFilter(
    ...
    : FramedFilter(env, inputSource),
      ...
      fFrameCount(0),
      fNextIndexRecordNum(0),
      fNextTSPacketNum(0),
      fCurrentTSPacketNum((unsigned long)(-1)),
      fUseSavedFrameNextTime(False) {
...
Boolean MPEG2TransportStreamTrickModeFilter::seekTo(
        unsigned long tsPacketNumber, unsigned long indexRecordNumber) {
    seekToTransportPacket(tsPacketNumber);
    fNextIndexRecordNum = indexRecordNumber;
    return True;
}

#define isIFrameStart(type)                           \
    ((type) == 0x81 /*actually, a VSH*/ ||            \
     (type) == 0x85 /*actually, a SPS, for H.264*/ || \
     (type) == 0x8B /*actually, a VPS, for H.265*/)
// This relies upon I-frames always being preceded by a VSH+GOP (for MPEG-2
// data), by a SPS (for H.264 data), or by a VPS (for H.265 data)
#define isNonIFrameStart(type)                         \
    ((type) == 0x83 || (type) == 0x88 /*for H.264*/ || \
     (type) == 0x8E /*for H.265*/)
...
void MPEG2TransportStreamTrickModeFilter::doGetNextFrame() {
    ...
    while (1) {
        ...
        // This tells us the type of frame this data is, which Transport Stream
        // packet (from the input source) the data comes from, and where in the
        // Transport Stream packet it comes from:
        u_int8_t recordType;
        float recordPCR;
        Boolean endOfIndexFile = False;
        if (!fIndexFile->readIndexRecordValues(
                    fNextIndexRecordNum, fDesiredTSPacketNum,
                    fDesiredDataOffset, fDesiredDataSize, recordPCR,
                    recordType)) {
        ...
        //    fprintf(stderr, "#####read index record %ld: ts %ld: %c, PCR
        //    %f\n", fNextIndexRecordNum, fDesiredTSPacketNum,
        //    isIFrameStart(recordType) ? 'I' : isNonIFrameStart(recordType) ?
        //    'j' : 'x', recordPCR);
        fNextIndexRecordNum +=
                (fState == DELIVERING_SAVED_FRAME) ? 1 : fDirection;

        // Handle this index record, depending on the record type and our
        // current state:
        switch (fState) {
            case SKIPPING_FRAME:
            case SAVING_AND_DELIVERING_FRAME: {
                //      if (fState == SKIPPING_FRAME) fprintf(stderr,
                //      "\tSKIPPING_FRAME\n"); else fprintf(stderr,
                //      "\tSAVING_AND_DELIVERING_FRAME\n");//#####
                if (isIFrameStart(recordType)) {
                    // Save a record of this frame:
                    fSavedFrameIndexRecordStart =
                            fNextIndexRecordNum - fDirection;
                    ...
                    if ((fFrameCount++) % fScale == 0 &&
                        fUseSavedFrameNextTime) {
                        // A frame is due now, so begin delivering the one that
                        // we had saved: (This relies on the index records
                        // having begun with an I-frame.)
                        fFrameCount = 1;  // reset to avoid overflow
                        fState = DELIVERING_SAVED_FRAME;
...
void MPEG2TransportStreamTrickModeFilter::attemptDeliveryToClient() {
    if (fCurrentTSPacketNum == fDesiredTSPacketNum) {
        ...
        //    fCurrentTSPacketNum, fDesiredDataOffset, fDesiredDataSize,
        //    fDesiredDataPCR);//#####
        ...
        fPresentationTime.tv_usec =
                (unsigned long)((deliveryPCR - fPresentationTime.tv_sec) *
                                1000000.0f);
...
void MPEG2TransportStreamTrickModeFilter ::afterGettingFrame(
        void* clientData,
        unsigned frameSize,
        unsigned /*numTruncatedBytes*/,
        struct timeval presentationTime,
        unsigned /*durationInMicroseconds*/) {
    MPEG2TransportStreamTrickModeFilter* filter =
            (MPEG2TransportStreamTrickModeFilter*)clientData;
