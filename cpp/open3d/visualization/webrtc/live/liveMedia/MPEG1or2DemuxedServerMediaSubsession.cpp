...
MPEG1or2DemuxedServerMediaSubsession ::createNew(MPEG1or2FileServerDemux& demux,
                                                 ...
                                                 Boolean iFramesOnly,
                                                 double vshPeriod) {
    return new MPEG1or2DemuxedServerMediaSubsession(
            demux, streamIdTag, reuseFirstSource, iFramesOnly, vshPeriod);
}
...
MPEG1or2DemuxedServerMediaSubsession ::MPEG1or2DemuxedServerMediaSubsession(
    ...
    : OnDemandServerMediaSubsession(demux.envir(), reuseFirstSource),
      fOurDemux(demux),
      fStreamIdTag(streamIdTag),
      fIFramesOnly(iFramesOnly),
      fVSHPeriod(vshPeriod) {}

MPEG1or2DemuxedServerMediaSubsession::~MPEG1or2DemuxedServerMediaSubsession() {}

FramedSource* MPEG1or2DemuxedServerMediaSubsession ::createNewStreamSource(
    ...
    do {
        ...
        if ((fStreamIdTag & 0xF0) == 0xC0 /*MPEG audio*/) {
            ...
            return MPEG1or2VideoStreamFramer::createNew(
                    envir(), es, fIFramesOnly, fVSHPeriod);
...
RTPSink* MPEG1or2DemuxedServerMediaSubsession ::createNewRTPSink(
    ...
    if ((fStreamIdTag & 0xF0) == 0xC0 /*MPEG audio*/) {
        ...
        // Get the sampling frequency from the audio source; use it for the RTP
        // frequency:
...
void MPEG1or2DemuxedServerMediaSubsession ::seekStreamSource(
        FramedSource* inputSource,
        double& seekNPT,
    ...
    unsigned absBytePosition =
            dur == 0.0 ? 0 : (unsigned)((seekNPT / dur) * size);
    ...
    if ((fStreamIdTag & 0xF0) == 0xC0 /*MPEG audio*/) {
        ...
        MPEG1or2VideoStreamFramer* framer =
                (MPEG1or2VideoStreamFramer*)inputSource;
        framer->flushInput();
    }

    // "inputSource" is a filter; its input source is the original elem stream
    // source:
    MPEG1or2DemuxedElementaryStream* elemStreamSource =
            (MPEG1or2DemuxedElementaryStream*)(((FramedFilter*)inputSource)
                                                       ->inputSource());
    ...
    // Note: We can make that cast, because we know that the demux was
    // originally created from a "ByteStreamFileSource".
