...
AMRAudioFileSource ::AMRAudioFileSource(UsageEnvironment& env,
                                        FILE* fid,
                                        Boolean isWideband,
...
void AMRAudioFileSource::doGetNextFrame() {
...
#ifdef DEBUG
            fprintf(stderr,
                    "Invalid frame header 0x%02x (padding bits (0x83) are not "
                    "zero)\n",
                    fLastFrameHeader);
#endif
        } else {
...
#ifdef DEBUG
                fprintf(stderr,
                        "Valid frame header 0x%02x -> ft %d -> frame size %d\n",
    ...
    fFrameSize *=
            fNumChannels;  // because multiple channels make up a frame-block
