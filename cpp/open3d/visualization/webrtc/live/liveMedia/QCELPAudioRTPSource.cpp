...
private:
    RawQCELPRTPSource(UsageEnvironment& env,
                      Groupsock* RTPgs,
                      unsigned char rtpPayloadFormat,
                      unsigned rtpTimestampFrequency);
...
FramedSource* QCELPAudioRTPSource::createNew(UsageEnvironment& env,
    ...
    resultRTPSource = rawRTPSource = RawQCELPRTPSource::createNew(
...
RawQCELPRTPSource* RawQCELPRTPSource::createNew(
...
Boolean RawQCELPRTPSource ::processSpecialHeader(
...
char const* RawQCELPRTPSource::MIMEtype() const { return "audio/QCELP"; }
...
#ifdef DEBUG
    fprintf(stderr,
            "QCELPBufferedPacket::nextEnclosedFrameSize(): frameSize: %d, "
            "dataSize: %d\n",
            frameSize, dataSize);
#endif
...
QCELPDeinterleaver* QCELPDeinterleaver::createNew(
        UsageEnvironment& env, RawQCELPRTPSource* inputSource) {
...
QCELPDeinterleaver::~QCELPDeinterleaver() { delete fDeinterleavingBuffer; }
...
void QCELPDeinterleaver ::afterGettingFrame(
        void* clientData,
        unsigned frameSize,
        unsigned /*numTruncatedBytes*/,
        struct timeval presentationTime,
...
void QCELPDeinterleaver ::afterGettingFrame1(unsigned frameSize,
    ...
    fDeinterleavingBuffer->deliverIncomingFrame(
            frameSize, source->interleaveL(), source->interleaveN(),
            source->frameIndex(), source->curPacketRTPSeqNum(),
            presentationTime);
...
void QCELPDeinterleavingBuffer ::deliverIncomingFrame(
        unsigned frameSize,
        unsigned char interleaveL,
        unsigned char interleaveN,
        unsigned char frameIndex,
...
#ifdef DEBUG
        fprintf(stderr,
                "QCELPDeinterleavingBuffer::deliverIncomingFrame() param "
                "sanity check failed (%d,%d,%d,%d)\n",
    ...
    unsigned uSecIncrement =
            (frameIndex - 1) * (interleaveL + 1) * uSecsPerFrame;
...
Boolean QCELPDeinterleavingBuffer ::retrieveFrame(
        unsigned char* to,
        unsigned maxSize,
        unsigned& resultFrameSize,
        unsigned& resultNumTruncatedBytes,
