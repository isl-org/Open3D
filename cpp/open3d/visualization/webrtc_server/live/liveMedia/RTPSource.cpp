...
Boolean RTPSource::isRTPSource() const { return True; }

RTPSource::RTPSource(UsageEnvironment& env,
                     Groupsock* RTPgs,
...
RTPSource::~RTPSource() { delete fReceptionStatsDB; }
...
void RTPReceptionStatsDB ::noteIncomingPacket(
    ...
    stats->noteIncomingPacket(seqNum, rtpTimestamp, timestampFrequency,
                              useForJitterCalculation, resultPresentationTime,
...
RTPReceptionStatsDB::Iterator::~Iterator() { delete fIter; }

RTPReceptionStats* RTPReceptionStatsDB::Iterator::next(
        Boolean includeInactiveSources) {
...
RTPReceptionStats::RTPReceptionStats(u_int32_t SSRC) { init(SSRC); }
...
void RTPReceptionStats::noteIncomingSR(u_int32_t ntpTimestampMSW,
    ...
    double microseconds =
            (ntpTimestampLSW * 15625.0) / 0x04000000;  // 10^6/2^32
...
unsigned RTPReceptionStats::jitter() const { return (unsigned)fJitter; }
