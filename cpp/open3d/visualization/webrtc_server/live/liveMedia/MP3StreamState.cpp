...
struct timeval MP3StreamState::currentFramePlayTime() const {
    ...
    unsigned const uSeconds = ((numSamples * 2 * MILLION) / freq + 1) /
...
unsigned MP3StreamState::getByteNumberFromPositionFraction(float fraction) {
    if (fHasXingTOC) {
        // The file is VBR, with a Xing TOC; use it to determine which byte to
        // seek to:
...
#ifdef DEBUG_ERRORS
        fprintf(stderr,
                "Insufficient buffer size for reading input frame (%d, need "
                "%d)\n",
...
void MP3StreamState::getAttributes(char* buffer, unsigned bufferSize) const {
    char const* formatStr =
            "bandwidth %d MPEGnumber %d MPEGlayer %d samplingFrequency %d "
            "isStereo %d playTime %d isVBR %d";
    unsigned fpt =
            (unsigned)(filePlayTime() + 0.5);  // rounds to nearest integer
#if defined(IRIX) || defined(ALPHA) || defined(_QNX4) || defined(IMN_PIM) || \
        defined(CRIS)
    /* snprintf() isn't defined, so just use sprintf() - ugh! */
    sprintf(buffer, formatStr, fr().bitrate, fr().isMPEG2 ? 2 : 1, fr().layer,
            fr().samplingFreq, fr().isStereo, fpt, fIsVBR);
#else
    snprintf(buffer, bufferSize, formatStr, fr().bitrate, fr().isMPEG2 ? 2 : 1,
             fr().layer, fr().samplingFreq, fr().isStereo, fpt, fIsVBR);
#endif
}
...
read_again:
    ...
    fr().hdr = ((unsigned long)hbuf[0] << 24) | ((unsigned long)hbuf[1] << 16) |
    ...
    init_resync:
        ...
        if ((fr().hdr & 0xffe00000) != 0xffe00000 ||
            ...
            || (fr().hdr & 0x00000003) !=
                       0x00000000  // 'emphasis' field unexpectedly set
        ) {
            ...
            */
            ...
            /* Check for ID3 hdr */
            if ((fr().hdr & 0xFFFFFF00) ==
                ('I' << 24) + ('D' << 16) + ('3' << 8)) {
                unsigned tagSize, bytesToSkip;
                unsigned char buf[1000];
                readFromStream(buf, 6); /* already read 4 */
                tagSize = ((buf[2] & 0x7F) << 21) + ((buf[3] & 0x7F) << 14) +
                          ((buf[4] & 0x7F) << 7) + (buf[5] & 0x7F);
                bytesToSkip = tagSize;
                while (bytesToSkip > 0) {
...
static Boolean socketIsReadable(int socket) {
    ...
    return result !=
           0;  // not > 0, because windows can return -1 for file sockets
}

static char watchVariable;

static void checkFunc(void* /*clientData*/) { watchVariable = ~0; }
...
static void waitUntilSocketIsReadable(UsageEnvironment& env, int socket) {
    while (!socketIsReadable(socket)) {
        ...
        env.taskScheduler().scheduleDelayedTask(
                usecsToDelay, (TaskFunc*)checkFunc, (void*)NULL);
        watchVariable = 0;
...
unsigned MP3StreamState::readFromStream(unsigned char* buf, unsigned numChars) {
    ...
    if (fFidIsReallyASocket) {
        ...
        do {
            ...
            int bytesRead = recv(sock, &((char*)buf)[totBytesRead],
                                 numChars - totBytesRead, 0);
