...
ADTSAudioFileSource* ADTSAudioFileSource::createNew(UsageEnvironment& env,
    ...
    do {
        ...
        // frame, to get the audio stream's parameters:
        ...
        u_int8_t sampling_frequency_index =
                (fixedHeader[2] & 0x3C) >> 2;  // 4 bits
        if (samplingFrequencyTable[sampling_frequency_index] == 0) {
            env.setResultMsg(
                    "Bad 'sampling_frequency_index' in first frame of ADTS "
                    "file");
            break;
        }
...
ADTSAudioFileSource ::ADTSAudioFileSource(UsageEnvironment& env,
    ...
    fuSecsPerFrame = (1024 /*samples-per-frame*/ * 1000000) /
...
ADTSAudioFileSource::~ADTSAudioFileSource() { CloseInputFile(fFid); }
...
void ADTSAudioFileSource::doGetNextFrame() {
    ...
    fprintf(stderr,
            "Read frame: syncword 0x%x, protection_absent %d, frame_length "
            "%d\n",
