...
ServerMediaSubsession* MPEG1or2FileServerDemux::newVideoServerMediaSubsession(
        Boolean iFramesOnly, double vshPeriod) {
    return MPEG1or2DemuxedServerMediaSubsession::createNew(
            *this, 0xE0, fReuseFirstSource, iFramesOnly, vshPeriod);
}
...
MPEG1or2DemuxedElementaryStream* MPEG1or2FileServerDemux::newElementaryStream(
        unsigned clientSessionId, u_int8_t streamIdTag) {
    MPEG1or2Demux* demuxToUse;
    if (clientSessionId == 0) {
        // 'Session 0' is treated especially, because its audio & video streams
        // are created and destroyed one-at-a-time, rather than both streams
        // being created, and then (later) both streams being destroyed (as is
        // the case for other ('real') session ids).  Because of this, a
        // separate demux is used for session 0, and its deletion is managed by
        // us, rather than happening automatically.
        ...
        // First, check whether this is a new client session.  If so, create a
        // new demux for it:
        if (clientSessionId != fLastClientSessionId) {
            ...
            fLastCreatedDemux =
                    MPEG1or2Demux::createNew(envir(), fileSource, True);
            // Note: We tell the demux to delete itself when its last
            // elementary stream is deleted.
            fLastClientSessionId = clientSessionId;
            // Note: This code relies upon the fact that the creation of streams
            // for different client sessions do not overlap - so one
            // "MPEG1or2Demux" is used at a time.
        }
...
static float MPEG1or2ProgramStreamFileDuration(UsageEnvironment& env,
    ...
    do {
        ...
        MPEG1or2Demux* baseDemux =
                MPEG1or2Demux::createNew(env, dataSource, True);
        ...
        // Take the difference between these time codes as being the file
        // duration:
...
#define MFSD_DUMMY_SINK_BUFFER_SIZE \
    (6 + 65535) /* large enough for a PES packet */
...
static Boolean getMPEG1or2TimeCode(FramedSource* dataSource,
    ...
    sink.startPlaying(*dataSource,
                      (MediaSink::afterPlayingFunc*)afterPlayingMFSD_DummySink,
                      &sink);
...
MFSD_DummySink::MFSD_DummySink(MPEG1or2Demux& demux,
                               Boolean returnFirstSeenCode)
    : MediaSink(demux.envir()),
      watchVariable(0),
      fOurDemux(demux),
      fReturnFirstSeenCode(returnFirstSeenCode) {}
...
Boolean MFSD_DummySink::continuePlaying() {
    ...
    fSource->getNextFrame(fBuf, sizeof fBuf, afterGettingFrame, this,
                          onSourceClosure, this);
    return True;
}

void MFSD_DummySink::afterGettingFrame(void* clientData,
                                       unsigned /*frameSize*/,
                                       unsigned /*numTruncatedBytes*/,
                                       struct timeval /*presentationTime*/,
                                       unsigned /*durationInMicroseconds*/) {
...
void MFSD_DummySink::afterGettingFrame1() {
    if (fReturnFirstSeenCode && fOurDemux.lastSeenSCR().isValid) {
        // We were asked to return the first SCR that we saw, and we've seen
        // one, so we're done.  (Handle this as if the input source had closed.)
        onSourceClosure();
        return;
    }
