...
QuickTimeGenericRTPSource* QuickTimeGenericRTPSource::createNew(
...
Boolean QuickTimeGenericRTPSource ::processSpecialHeader(
...
#endif
        unsigned payloadDescriptionLength =
                (headerStart[2] << 8) | headerStart[3];
        headerStart += 4;

#ifdef DEBUG
        fprintf(stderr,
                "\tK: %d, F: %d, A: %d, Z: %d, payloadDescriptionLength: %d\n",
...
#ifdef DEBUG
        fprintf(stderr,
                "\tmediaType: '%c%c%c%c', timescale: %d, %d bytes of TLVs "
                "left\n",
                mediaType >> 24, (mediaType & 0xFF0000) >> 16,
                (mediaType & 0xFF00) >> 8, mediaType & 0xFF, qtState.timescale,
        ...
        while (payloadDescriptionLength > 3) {
...
#ifdef DEBUG
            fprintf(stderr,
                    "\t\tTLV '%c%c', length %d, leaving %d remaining bytes\n",
                    tlvType >> 8, tlvType & 0xFF, tlvLength,
            ...
            switch (tlvType) {
                case ('s' << 8 | 'd'): {  // session description atom
                    // Sanity check: the first 4 bytes of this must equal
                    // "tlvLength":
                    unsigned atomLength =
                            (headerStart[0] << 24) | (headerStart[1] << 16) |
                            (headerStart[2] << 8) | (headerStart[3]);
...
QTGenericBufferedPacket ::QTGenericBufferedPacket(
        QuickTimeGenericRTPSource& ourSource)
...
BufferedPacket* QTGenericBufferedPacketFactory ::createNewPacket(
        MultiFramedRTPSource* ourSource) {
    return new QTGenericBufferedPacket(
            (QuickTimeGenericRTPSource&)(*ourSource));
}
