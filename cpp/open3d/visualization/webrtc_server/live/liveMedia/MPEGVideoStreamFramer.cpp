...
void MPEGVideoStreamFramer::reset() {
    fPictureCount = 0;
    fPictureEndMarker = True;  // So that we start looking as if we'd just ended
                               // an 'access unit'
...
void MPEGVideoStreamFramer ::computePresentationTime(
    ...
    double pictureTime = fFrameRate == 0.0
                                 ? 0.0
                                 : (tc.pictures + fPicturesAdjustment +
...
#ifdef DEBUG
        fprintf(stderr,
                "%d bytes @%u.%06d, fDurationInMicroseconds: %d "
                "((%d*1000000)/%f)\n",
                acquiredFrameSize, fPresentationTime.tv_sec,
                fPresentationTime.tv_usec, fDurationInMicroseconds,
                fPictureCount, fFrameRate);
#endif
        fPictureCount = 0;
