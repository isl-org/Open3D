...
Boolean MultiFramedRTPSource ::processSpecialHeader(
...
void MultiFramedRTPSource::doGetNextFrame1() {
    while (fNeedDelivery) {
        ...
        unsigned frameSize;
        nextPacket->use(
                fTo, fMaxSize, frameSize, fNumTruncatedBytes,
        ...
        if (fCurrentPacketCompletesFrame && fFrameSize > 0) {
            ...
            if (fNumTruncatedBytes > 0) {
                envir() << "MultiFramedRTPSource::doGetNextFrame1(): The total "
                           "received frame size exceeds the client's buffer "
                           "size ("
                        << fSavedMaxSize << ").  " << fNumTruncatedBytes
                        << " bytes of trailing data will be dropped!\n";
            }
            // Call our own 'after getting' function, so that the downstream
            // object can consume the data:
            if (fReorderingBuffer->isEmpty()) {
                // Common case optimization: There are no more queued incoming
                // packets, so this code will not get executed again without
                // having first returned to the event loop.  Call our 'after
                // getting' function directly, because there's no risk of a long
                // chain of recursion (and thus stack overflow):
                afterGetting(this);
            } else {
                // Special case: Call our 'after getting' function via the event
                // loop.
...
void MultiFramedRTPSource::networkReadHandler1() {
    ...
    if (bPacket == NULL) {
        // Normal case: Get a free BufferedPacket descriptor to hold the new
        // network packet:
    ...
    do {
        ...
        if (!bPacket->fillInData(fRTPInterface, fromAddress,
            ...
            if (bPacket->bytesAvailable() == 0) {  // should not happen??
                envir() << "MultiFramedRTPSource internal error: Hit limit "
                           "when reading incoming packet over TCP\n";
            }
            fPacketReadInProgress = NULL;
            break;
        }
        if (packetReadWasIncomplete) {
            // We need additional read(s) before we can process the incoming
            // packet:
        ...
        if (rtpPayloadType != rtpPayloadFormat()) {
            if (fRTCPInstanceForMultiplexedRTCPPackets != NULL &&
                ...
                // This is a multiplexed RTCP packet, and we've been asked to
                // deliver such packets. Do so now:
        ...
        if (rtpSSRC != fLastReceivedSSRC) {
            // The SSRC of incoming packets has changed.  Unfortunately we don't
            // yet handle streams that contain multiple SSRCs, but we can handle
            // a single-SSRC stream where the SSRC changes occasionally:
...
ReorderingPacketBuffer ::ReorderingPacketBuffer(
    ...
    : fThresholdTime(100000) /* default reordering threshold: 100 ms */,
      ...
      fSavedPacket(NULL),
      fSavedPacketFree(True) {
    fPacketFactory = (packetFactory == NULL) ? (new BufferedPacketFactory)
                                             : packetFactory;
}
...
Boolean ReorderingPacketBuffer::storePacket(BufferedPacket* bPacket) {
    ...
    if (fTailPacket == NULL) {
        // Common case: There are no packets in the queue; this will be the
        // first one:
    ...
    if (seqNumLT(fTailPacket->rtpSeqNo(), rtpSeqNo)) {
        // The next-most common case: There are packets already in the queue;
        // this packet arrived in order => put it at the tail:
        bPacket->nextPacket() = NULL;
...
BufferedPacket* ReorderingPacketBuffer ::getNextCompletedPacket(
    ...
    if (fThresholdTime == 0) {
        ...
        unsigned uSecondsSinceReceived =
                (timeNow.tv_sec - fHeadPacket->timeReceived().tv_sec) *
                        1000000 +
