...
public:
    ...
    unsigned& frameIndex() {
        return fFrameIndex;
    }  // index of frame-block within pkt
...
AMRAudioSource* AMRAudioRTPSource::createNew(UsageEnvironment& env,
    ...
    if (robustSortingOrder) {
        env << "AMRAudioRTPSource::createNew(): 'Robust sorting order' was "
               "specified, but we don't yet support this!\n";
        return NULL;
    } else if (numChannels > MAX_NUM_CHANNELS) {
        env << "AMRAudioRTPSource::createNew(): The \"number of channels\" "
               "parameter ("
    ...
    if (!isOctetAligned) {
        if (interleaving > 0 || robustSortingOrder || CRCsArePresent) {
            env << "AMRAudioRTPSource::createNew(): 'Bandwidth-efficient mode' "
                   "was specified, along with interleaving, 'robust sorting "
                   "order', and/or CRCs, so we assume 'octet-aligned mode' "
                   "instead.\n";
            isOctetAligned = True;
        }
    }
    ...
    resultRTPSource = rawRTPSource = RawAMRRTPSource::createNew(
            env, RTPgs, rtpPayloadFormat, isWideband, isOctetAligned,
            isInterleaved, CRCsArePresent);
    if (resultRTPSource == NULL) return NULL;

    AMRDeinterleaver* deinterleaver = AMRDeinterleaver::createNew(
            env, isWideband, numChannels, maxInterleaveGroupSize, rawRTPSource);
...
RawAMRRTPSource* RawAMRRTPSource::createNew(UsageEnvironment& env,
                                            ...
                                            Boolean isInterleaved,
                                            Boolean CRCsArePresent) {
    return new RawAMRRTPSource(env, RTPgs, rtpPayloadFormat, isWideband,
...
RawAMRRTPSource ::RawAMRRTPSource(UsageEnvironment& env,
    ...
    : MultiFramedRTPSource(env,
      ...
      fIsOctetAligned(isOctetAligned),
      fIsInterleaved(isInterleaved),
      fCRCsArePresent(CRCsArePresent),
      fILL(0),
      fILP(0),
      fTOCSize(0),
      fTOC(NULL),
...
RawAMRRTPSource::~RawAMRRTPSource() { delete[] fTOC; }
...
Boolean RawAMRRTPSource ::processSpecialHeader(
...
unsigned AMRBufferedPacket::nextEnclosedFrameSize(unsigned char*& framePtr,
    ...
    unsigned char const FT = (tocByte & 0x78) >> 3;
    // ASSERT: FT < 16
    unsigned short frameSize = fOurSource.isWideband()
                                       ? frameBytesFromFTWideband[FT]
                                       : frameBytesFromFT[FT];
    if (frameSize == FT_INVALID) {
        ...
        fOurSource.envir()
                ...
                << FT << "\n";
        frameSize =
                0;  // This probably messes up the rest of this packet, but...
    }
#ifdef DEBUG
    fprintf(stderr,
            "AMRBufferedPacket::nextEnclosedFrameSize(): frame #: %d, FT: %d, "
            "isWideband: %d => frameSize: %d (dataSize: %d)\n",
...
BufferedPacket* AMRBufferedPacketFactory ::createNewPacket(
...
public:
    AMRDeinterleavingBuffer(unsigned numChannels,
                            unsigned maxInterleaveGroupSize);
    ...
    void deliverIncomingFrame(unsigned frameSize,
                              ...
                              struct timeval presentationTime);
    Boolean retrieveFrame(unsigned char* to,
                          unsigned maxSize,
...
AMRDeinterleaver* AMRDeinterleaver ::createNew(UsageEnvironment& env,
                                               ...
                                               unsigned maxInterleaveGroupSize,
                                               RawAMRRTPSource* inputSource) {
    return new AMRDeinterleaver(env, isWideband, numChannels,
                                maxInterleaveGroupSize, inputSource);
}
...
void AMRDeinterleaver::doGetNextFrame() {
    ...
    if (fDeinterleavingBuffer->retrieveFrame(
                fTo, fMaxSize, fFrameSize, fNumTruncatedBytes, fLastFrameHeader,
                fPresentationTime, fInputSource->isSynchronized())) {
        // Success!
        fNeedAFrame = False;
...
void AMRDeinterleaver ::afterGettingFrame1(unsigned frameSize,
    ...
    fDeinterleavingBuffer->deliverIncomingFrame(frameSize, source,
                                                presentationTime);
...
AMRDeinterleavingBuffer ::AMRDeinterleavingBuffer(
        unsigned numChannels, unsigned maxInterleaveGroupSize)
    : fNumChannels(numChannels),
      fMaxInterleaveGroupSize(maxInterleaveGroupSize),
      fIncomingBankId(0),
      fIncomingBinMax(0),
      fOutgoingBinMax(0),
      fNextOutgoingBin(0),
...
void AMRDeinterleavingBuffer ::deliverIncomingFrame(
        unsigned frameSize,
        RawAMRRTPSource* source,
        struct timeval presentationTime) {
    fILL = source->ILL();
...
#ifdef DEBUG
        fprintf(stderr,
                "AMRDeinterleavingBuffer::deliverIncomingFrame() param sanity "
                "check failed (%d,%d,%d,%d)\n",
                frameSize, fILL, ILP, frameIndex);
#endif
        source->envir().internalError();
    }

    --frameIndex;  // because it was incremented by the source when this frame
                   // was read
    u_int8_t frameHeader;
...
#ifdef DEBUG
        fprintf(stderr,
                "AMRDeinterleavingBuffer::deliverIncomingFrame(): new "
                "interleave group\n");
#endif
    ...
    unsigned const binNumber =
            ((ILP + frameBlockIndex * (fILL + 1)) * fNumChannels +
             frameWithinFrameBlock) %
            fMaxInterleaveGroupSize;  // the % is for sanity
#ifdef DEBUG
    fprintf(stderr,
            "AMRDeinterleavingBuffer::deliverIncomingFrame(): frameIndex %d "
            "(%d,%d) put in bank %d, bin %d (%d): size %d, header 0x%02x, "
            "presentationTime %lu.%06ld\n",
            frameIndex, frameBlockIndex, frameWithinFrameBlock, fIncomingBankId,
            binNumber, fMaxInterleaveGroupSize, frameSize, frameHeader,
    ...
    inBin.fIsSynchronized =
            ((RTPSource*)source)->RTPSource::hasBeenSynchronizedUsingRTCP();
...
Boolean AMRDeinterleavingBuffer ::retrieveFrame(
        unsigned char* to,
        unsigned maxSize,
        unsigned& resultFrameSize,
        unsigned& resultNumTruncatedBytes,
    ...
    if (outBin.fIsSynchronized) {
        // Don't consider the outgoing frame to be synchronized until we've
        // received at least a complete interleave cycle of synchronized frames.
        // This ensures that the receiver will be getting all synchronized
        // frames from now on.
...
#ifdef DEBUG
    fprintf(stderr,
            "AMRDeinterleavingBuffer::retrieveFrame(): from bank %d, bin %d: "
            "size %d, header 0x%02x, presentationTime %lu.%06ld\n",
            fIncomingBankId ^ 1, fNextOutgoingBin, resultFrameSize,
            resultFrameHeader, resultPresentationTime.tv_sec,
...
static void unpackBandwidthEfficientData(BufferedPacket* packet,
    ...
    unsigned const toBufferSize =
            2 * packet->dataSize();  // conservatively large
...
#ifdef DEBUG
            fprintf(stderr,
                    "\tWarning: Unpacking frame %d of %d: want %d bits, but "
    ...
    packet->removePadding(
            packet->dataSize());  // throws away current packet data
