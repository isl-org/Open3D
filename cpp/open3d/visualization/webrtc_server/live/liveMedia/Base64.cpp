...
unsigned char* base64Decode(char const* in,
    ...
    unsigned char* out =
            new unsigned char[inSize + 1];  // ensures we have enough space
    int k = 0;
    int paddingCount = 0;
    ...
    for (int j = 0; j < jMax; j += 4) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if ((outTmp[i] & 0x80) != 0)
                outTmp[i] = 0;  // this happens only if there was an invalid
                                // character; pretend that it was 'A'
        }
...
char* base64Encode(char const* origSigned, unsigned origLength) {
    ...
    for (i = 0; i < numOrig24BitValues; ++i) {
        ...
        result[4 * i + 1] = base64Char[(((orig[3 * i] & 0x3) << 4) |
                                        (orig[3 * i + 1] >> 4)) &
                                       0x3F];
        result[4 * i + 2] =
                base64Char[((orig[3 * i + 1] << 2) | (orig[3 * i + 2] >> 6)) &
                           0x3F];
