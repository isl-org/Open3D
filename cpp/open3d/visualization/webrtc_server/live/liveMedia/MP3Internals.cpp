...
static unsigned const live_tabsel[2][3][16] = {
        {{32, 32, 64, 96, 128, 160, 192, 224, 256, 288, 320, 352, 384, 416, 448,
          448},
         {32, 32, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 384,
          384},
         {32, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320,
          320}},

        {{32, 32, 48, 56, 64, 80, 96, 112, 128, 144, 160, 176, 192, 224, 256,
          256},
...
static unsigned updateSideInfoSizes(MP3SideInfo& sideInfo,
    ...
    fprintf(stderr, "updateSideInfoSizes (allowed: %d): %d->%d, %d->%d\n",
            allowedNumBits, p23L0 + p23L0Trunc, p23L0, p23L1 + p23L1Trunc,
            p23L1);
#endif
...
#ifdef DEBUG
    fprintf(stderr,
            "mp3GetADUInfoFromFrame: hdr: %08x, frameSize: %d, "
            "part2_3_lengths: %d,%d,%d,%d, aduSize: %d, backpointer: %d\n",
       ...
       if (ms_stereo) gr_info.pow2gain += 2;
            ...
            if (gr_info.window_switching_flag) {
                ...
                for (i = 0; i < 3; i++) {
                    ...
                    gr_info.full_gain[i] = gr_info.pow2gain +
                                           ((gr_info.subblock_gain[i]) << 3);
                }

#ifdef DEBUG_ERRORS
                if (gr_info.block_type == 0) {
                    fprintf(stderr,
                            "Blocktype == 0 and window-switching == 1 not "
                            "allowed.\n");
                }
#endif
...
unsigned TranscodeMP3ADU(unsigned char const* fromPtr,
    ...
    unsigned desiredOutAduSize /*=inAduSize*outAveAduSize/inAveAduSize*/
            = (2 * inAduSize * outAveAduSize + inAveAduSize) /
              (2 * inAveAduSize);
