...
#define INPUT_BUFFER_SIZE     \
    (SIMPLE_PES_HEADER_SIZE + \
     2 * MPEG2TransportStreamFromESSource::maxInputESFrameSize)
#define LOW_WATER_MARK \
    1000  // <= MPEG2TransportStreamFromESSource::maxInputESFrameSize
...
unsigned MPEG2TransportStreamFromESSource::maxInputESFrameSize =
        100000;  // bytes

MPEG2TransportStreamFromESSource* MPEG2TransportStreamFromESSource ::createNew(
        UsageEnvironment& env) {
    return new MPEG2TransportStreamFromESSource(env);
}

void MPEG2TransportStreamFromESSource ::addNewVideoSource(
        FramedSource* inputSource, int mpegVersion, int16_t PID) {
...
void MPEG2TransportStreamFromESSource ::addNewAudioSource(
        FramedSource* inputSource, int mpegVersion, int16_t PID) {
...
MPEG2TransportStreamFromESSource ::MPEG2TransportStreamFromESSource(
    ...
    : MPEG2TransportStreamMultiplexor(env),
      fInputSources(NULL),
      fVideoSourceCounter(0),
      fAudioSourceCounter(0),
...
void MPEG2TransportStreamFromESSource ::awaitNewBuffer(
...
void MPEG2TransportStreamFromESSource ::addNewInputSource(
        FramedSource* inputSource,
        u_int8_t streamId,
        int mpegVersion,
        int16_t PID) {
...
InputESSourceRecord ::InputESSourceRecord(
        MPEG2TransportStreamFromESSource& parent,
        FramedSource* inputSource,
        u_int8_t streamId,
        int mpegVersion,
        InputESSourceRecord* next,
...
void InputESSourceRecord::askForNewData() {
    ...
    if (fInputBufferBytesAvailable == 0) {
        ...
        fInputBuffer[4] = 0;
        fInputBuffer[5] = 0;  // fill in later with the length
        fInputBuffer[6] = 0x80;
    ...
    if (fInputBufferBytesAvailable < LOW_WATER_MARK &&
        ...
        fInputSource->getNextFrame(
                ...
                afterGettingFrame, this, FramedSource::handleClosure, &fParent);
    }
}

Boolean InputESSourceRecord::deliverBufferToClient() {
    if (fInputBufferInUse || fInputBufferBytesAvailable < LOW_WATER_MARK)
        return False;
    ...
    if (PES_packet_length > 0xFFFF) {
        // Set the PES_packet_length field to 0.  This indicates an unbounded
        // length (see ISO 13818-1, 2.4.3.7)
        PES_packet_length = 0;
    }
    ...
    fInputBuffer[9] =
            0x20 | (fSCR.highBit << 3) | (fSCR.remainingBits >> 29) | 0x01;
...
void InputESSourceRecord ::afterGettingFrame(
        void* clientData,
        unsigned frameSize,
        unsigned numTruncatedBytes,
        struct timeval presentationTime,
...
void InputESSourceRecord ::afterGettingFrame1(unsigned frameSize,
    ...
    if (numTruncatedBytes > 0) {
        fParent.envir() << "MPEG2TransportStreamFromESSource: input buffer too "
                           "small; increase "
                           "\"MPEG2TransportStreamFromESSource::"
                           "maxInputESFrameSize\" by at least "
                        << numTruncatedBytes << " bytes!\n";
    }

    if (fInputBufferBytesAvailable == SIMPLE_PES_HEADER_SIZE) {
        // Use this presentationTime for our SCR:
        fSCR.highBit = ((presentationTime.tv_sec * 45000 +
                         (presentationTime.tv_usec * 9) / 200) &
                        0x80000000) != 0;
        fSCR.remainingBits = presentationTime.tv_sec * 90000 +
                             (presentationTime.tv_usec * 9) / 100;
        fSCR.extension = (presentationTime.tv_usec * 9) % 100;
#ifdef DEBUG_SCR
        fprintf(stderr,
                "PES header: stream_id 0x%02x, pts: %u.%06u => SCR "
                "0x%x%08x:%03x\n",
                fStreamId, (unsigned)presentationTime.tv_sec,
                (unsigned)presentationTime.tv_usec, fSCR.highBit,
                fSCR.remainingBits, fSCR.extension);
#endif
    }
