...
FramedSource* MP3ADUinterleaverBase::getInputSource(
        UsageEnvironment& env, char const* inputSourceName) {
    FramedSource* inputSource;
...
MP3ADUinterleaver::~MP3ADUinterleaver() { delete fFrames; }

MP3ADUinterleaver* MP3ADUinterleaver::createNew(
...
void MP3ADUinterleaver::doGetNextFrame() {
    ...
    if (fFrames->haveReleaseableFrame()) {
        ...
        fInputSource->getNextFrame(dataPtr, bytesAvailable,
                                   &MP3ADUinterleaverBase::afterGettingFrame,
...
MP3ADUdeinterleaver::~MP3ADUdeinterleaver() { delete fFrames; }
...
void MP3ADUdeinterleaver::doGetNextFrame() {
    ...
    NOTE:
        This code no longer works, because it uses synchronous reads,
                which are no longer
                        supported.static unsigned const framesPerPacket = 3;
...
InterleavingFrames::InterleavingFrames(unsigned maxCycleSize)
    : fMaxCycleSize(maxCycleSize),
      fNextIndexToRelease(0),
      fDescriptors(new InterleavingFrameDescriptor[maxCycleSize]) {}
InterleavingFrames::~InterleavingFrames() { delete[] fDescriptors; }
...
void InterleavingFrames::getReleasingFrameParams(
        unsigned char index,
        unsigned char*& dataPtr,
        unsigned& bytesInUse,
        struct timeval& presentationTime,
...
DeinterleavingFrames::~DeinterleavingFrames() { delete[] fDescriptors; }
...
Boolean DeinterleavingFrames::haveReleaseableFrame() {
    if (!fHaveEndedCycle) {
        ...
        if (fNextIndexToRelease >= fMaxIndexSeen) {
            // No more frames are available from the cycle that we just ended,
            // so clear out all previously stored frames, then make available
...
void DeinterleavingFrames ::getIncomingFrameParamsAfter(
        unsigned frameSize,
        struct timeval presentationTime,
        unsigned durationInMicroseconds,
        unsigned char& icc,
...
void DeinterleavingFrames::getReleasingFrameParams(
        unsigned char*& dataPtr,
        unsigned& bytesInUse,
        struct timeval& presentationTime,
        unsigned& durationInMicroseconds) {
...
void DeinterleavingFrames::startNewCycle() { fHaveEndedCycle = True; }
