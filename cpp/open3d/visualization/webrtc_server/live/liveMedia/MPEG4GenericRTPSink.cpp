...
MPEG4GenericRTPSink ::MPEG4GenericRTPSink(UsageEnvironment& env,
    ...
    : MultiFramedRTPSink(env,
      ...
      fMPEG4Mode(strDup(mpeg4Mode)),
      fConfigString(strDup(configString)) {
    ...
    if (mpeg4Mode == NULL) {
        ...
        if (strcmp(m, "aac-hbr") != 0) {
            env << "MPEG4GenericRTPSink error: Unknown \"mpeg4Mode\" "
                   "parameter: \""
                << mpeg4Mode << "\"\n";
        }
        delete[] m;
...
MPEG4GenericRTPSink* MPEG4GenericRTPSink::createNew(
    ...
    return new MPEG4GenericRTPSink(env, RTPgs, rtpPayloadFormat,
                                   ...
                                   mpeg4Mode, configString, numChannels);
}
...
void MPEG4GenericRTPSink ::doSpecialFrameHandling(
...
unsigned MPEG4GenericRTPSink::specialHeaderSize() const { return 2 + 2; }

char const* MPEG4GenericRTPSink::sdpMediaType() const {
    return fSDPMediaTypeString;
}

char const* MPEG4GenericRTPSink::auxSDPLine() { return fFmtpSDPLine; }
