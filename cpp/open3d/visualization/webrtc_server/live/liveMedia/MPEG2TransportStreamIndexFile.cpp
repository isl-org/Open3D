...
MPEG2TransportStreamIndexFile ::MPEG2TransportStreamIndexFile(
    ...
    : Medium(env),
      fFileName(strDup(indexFileName)),
      fFid(NULL),
      fMPEGVersion(0),
      fCurrentIndexRecordNum(0),
      fCachedPCR(0.0f),
      fCachedTSPacketNumber(0),
      fNumIndexRecords(0) {
...
MPEG2TransportStreamIndexFile* MPEG2TransportStreamIndexFile ::createNew(
        UsageEnvironment& env, char const* indexFileName) {
...
void MPEG2TransportStreamIndexFile ::lookupPCRFromTSPacketNum(
    ...
    do {
        ...
        while (ixRight - ixLeft > 1 && tsLeft < tsPacketNumber &&
            ...
            unsigned long ixNew =
                    ixLeft + (unsigned long)(((tsPacketNumber - tsLeft) /
                                              (tsRight - tsLeft)) *
                                             (ixRight - ixLeft));
            if (ixNew == ixLeft || ixNew == ixRight) {
                // Use bisection instead:
        ...
        if (reverseToPreviousCleanPoint) {
            // "Rewind' until we reach the start of a Video Sequence or GOP
            // header:
            success = rewindToCleanPoint(ixFound);
        } else {
            success = True;
    ...
    if (success && readIndexRecord(ixFound)) {
        ...
        // An error occurred: Return the default values, for tsPacketNumber ==
        // 0:
        pcr = 0.0f;
        indexRecordNumber = 0;
    }
...
int MPEG2TransportStreamIndexFile::mpegVersion() {
    ...
    // Read the first index record, and figure out the MPEG version from its
    // type:
...
Boolean MPEG2TransportStreamIndexFile::rewindToCleanPoint(
    ...
    while (ixFound > 0) {
        ...
        // H.265, this is a VPS. For MPEG-2, this is a Video Sequence Header, or
        // a GOP.
        ...
        if ((recordType & 0x80) != 0) {  // This is the start of a 'frame'
            ...
            if (fMPEGVersion == 5) {     // H.264
                ...
                if (recordType == 1 /*VSH*/) {
                    ...
                    // Hack: If the preceding record is for a Video Sequence
                    // Header, then use it instead:
