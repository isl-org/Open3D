...
RTPSink::~RTPSink() {
    ...
    // so that the "fRTCPInterface" destructor doesn't turn off background read
    // handling (in case its 'groupsock' is being shared with something else
    // that does background read handling).
}
...
u_int32_t RTPSink::convertToRTPTimestamp(struct timeval tv) {
    ...
    timestampIncrement +=
            (u_int32_t)(fTimestampFrequency * (tv.tv_usec / 1000000.0) +
                        0.5);  // note: rounding
...
u_int32_t RTPSink::presetNextTimestamp() {
    ...
    if (!groupsockBeingUsed().hasMultipleDestinations()) {
        // Don't adjust the timestamp stream if we already have another
        // destination ongoing
...
#ifdef DEBUG_RR
        fprintf(stderr,
                "Adding new entry for SSRC %x in RTPTransmissionStatsDB\n",
                SSRC);
#endif
    }
...
unsigned RTPTransmissionStats::roundTripDelay() const {
    ...
    if (rawResult < 0) {
        // This can happen if there's clock drift between the sender and
        // receiver, and if the round-trip time was very small.
        rawResult = 0;
    }
