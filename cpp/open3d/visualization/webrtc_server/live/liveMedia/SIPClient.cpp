...
char* SIPClient::invite1(Authenticator* authenticator) {
    do {
        ...
        char* authenticatorStr = createAuthenticatorString(
                fWorkingAuthenticator, "INVITE", fURL);
        ...
        char const* const inviteSDPFmt =
                ...
                "m=audio %u RTP/AVP %u\r\n"
                "%s";
        unsigned inviteSDPFmtSize =
                strlen(inviteSDPFmt) + 20 /* max int len */ + 20 +
                fOurAddressStrSize + fApplicationNameSize + fOurAddressStrSize +
                5 /* max short len */ + 3 /* max char len */
                + rtpmapLineSize;
        delete[] fInviteSDPDescription;
        fInviteSDPDescription = new char[inviteSDPFmtSize];
        sprintf(fInviteSDPDescription, inviteSDPFmt, fCallId, fCSeq,
                fOurAddressStr, fApplicationName, fOurAddressStr,
                fClientStartPortNum, fDesiredAudioRTPPayloadFormat, rtpmapLine);
        ...
        char const* const cmdFmt =
                ...
                "Content-Length: %d\r\n\r\n"
                "%s";
        unsigned inviteCmdSize =
                strlen(cmdFmt) + fURLSize + 2 * fUserNameSize +
                fOurAddressStrSize + 20  /* max int len */
                + fOurAddressStrSize + 5 /* max port len */
                + fURLSize + fUserNameSize + fOurAddressStrSize + 5 + 20 +
                fOurAddressStrSize + 20 + strlen(authenticatorStr) +
                fUserAgentHeaderStrLen + 20 + inviteSDPSize;
        delete[] fInviteCmd;
    ...
    do {
        ...
        if (responseCode != 200) {
            if (responseCode >= 400 && responseCode <= 499 &&
                ...
                // "Proxy-Authenticate:" or "WWW-Authenticate:" line.  (Once we
                // compute a 'response' for "fWorkingAuthenticator", it can be
                ...
                while (1) {
                    ...
                    // ##### Check for the format of "Proxy-Authenticate:" lines
                    // from
                    // ##### known server types.
                    // ##### This is a crock! We should make the parsing more
                    // general
        ...
        if (bodyStart != NULL && contentLength >= 0) {
            ...
            if (contentLength > (int)numBodyBytes) {
                ...
                if (numExtraBytesNeeded > remainingBufferSize) {
                    char tmpBuf[200];
                    sprintf(tmpBuf,
                            "Read buffer size (%d) is too small for "
                            "\"Content-Length:\" %d (need a buffer size of >= "
                            "%d bytes\n",
                            readBufSize, contentLength,
                            readBufSize + numExtraBytesNeeded -
                                    remainingBufferSize);
                ...
                while (numExtraBytesNeeded > 0) {
                    ...
                    Boolean readSuccess = fOurSocket->handleRead(
                            (unsigned char*)ptr, numExtraBytesNeeded,
                            bytesRead2, fromAddr);
...
Boolean SIPClient::sendACK() {
    ...
    do {
        ...
        unsigned cmdSize = strlen(cmdFmt) + fURLSize + 2 * fUserNameSize +
                           fOurAddressStrSize + 20  /* max int len */
                           + fOurAddressStrSize + 5 /* max port len */
                           + fURLSize + fToTagStrSize + 20 +
                           fOurAddressStrSize + 20;
        cmd = new char[cmdSize];
        sprintf(cmd, cmdFmt, fURL, fUserName, fUserName, fOurAddressStr,
                fFromTag, fOurAddressStr, fOurPortNum, fURL, fToTagStr, fCallId,
                fOurAddressStr,
...
char* SIPClient::createAuthenticatorString(Authenticator const* authenticator,
    ...
    if (authenticator != NULL && authenticator->realm() != NULL &&
        ...
        unsigned authBufSize =
                strlen(authFmt) + strlen(authenticator->username()) +
