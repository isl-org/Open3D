...
H264VideoRTPSource* H264VideoRTPSource::createNew(
...
H264VideoRTPSource ::H264VideoRTPSource(UsageEnvironment& env,
                                        Groupsock* RTPgs,
                                        unsigned char rtpPayloadFormat,
                                        unsigned rtpTimestampFrequency)
...
Boolean H264VideoRTPSource ::processSpecialHeader(
    ...
    switch (fCurPacketNALUnitType) {
        ...
        case 29: {  // // FU-A or FU-B
            // For these NALUs, the first two bytes are the FU indicator and the
            // FU header. If the start bit is set, we reconstruct the original
            // NAL header into byte 1:
            ...
            if (startBit) {
                ...
                headerStart[1] =
                        (headerStart[0] & 0xE0) | (headerStart[1] & 0x1F);
                numBytesToSkip = 1;
            } else {
                // The start bit is not set, so we skip both the FU indicator
                // and header:
...
unsigned H264BufferedPacket ::nextEnclosedFrameSize(unsigned char*& framePtr,
    ...
    switch (fOurSource.fCurPacketNALUnitType) {
        ...
        case 26: {  // MTAP16
            // The first two bytes are NALU size.  The next three are the DOND
            // and TS offset:
            if (dataSize < 5) break;
            resultNALUSize = (framePtr[0] << 8) | framePtr[1];
            framePtr += 5;
        ...
        case 27: {  // MTAP24
            // The first two bytes are NALU size.  The next four are the DOND
            // and TS offset:
            if (dataSize < 6) break;
            resultNALUSize = (framePtr[0] << 8) | framePtr[1];
            framePtr += 6;
