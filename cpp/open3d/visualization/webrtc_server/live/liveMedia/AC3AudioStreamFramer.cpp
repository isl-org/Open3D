...
private:
    static void afterGettingSavedFrame(void* clientData,
                                       unsigned frameSize,
...
AC3AudioStreamFramer::~AC3AudioStreamFramer() { delete fParser; }

AC3AudioStreamFramer* AC3AudioStreamFramer::createNew(
...
void AC3AudioStreamFramer::flushInput() { fParser->flushInput(); }
...
void AC3AudioStreamFramer ::handleNewData(void* clientData,
                                          unsigned char* ptr,
                                          unsigned size,
                                          struct timeval /*presentationTime*/) {
...
void AC3AudioStreamFramer ::handleNewData(unsigned char* ptr, unsigned size) {
...
static int const kbpsTable[] = {32,  40,  48,  56,  64,  80,  96,
...
AC3AudioStreamParser ::AC3AudioStreamParser(AC3AudioStreamFramer* usingSource,
    ...
    : StreamParser(inputSource,
                   ...
                   usingSource,
                   &AC3AudioStreamFramer::handleNewData,
                   usingSource),
      fUsingSource(usingSource),
      fHaveParsedAFrame(False),
      fSavedFrame(NULL),
      fSavedFrameSize(0) {}
...
void AC3AudioStreamParser ::testStreamCode(unsigned char ourStreamCode,
                                           ...
                                           unsigned size) {
    if (ourStreamCode == 0)
        return;  // we assume that there's no stream code at the beginning of
                 // the data
...
void AC3AudioStreamParser ::afterGettingSavedFrame(
        void* clientData,
        unsigned frameSize,
        unsigned /*numTruncatedBytes*/,
        struct timeval /*presentationTime*/,
...
void AC3AudioStreamParser ::afterGettingSavedFrame1(unsigned frameSize) {
