...
NetAddressList::NetAddressList(char const* hostname)
    ...
    if (addr != INADDR_NONE) {
        // Yes, it was an IP address string.  Return a 1-element list with this
        // address:
        fNumAddresses = 1;
        ...
        fAddressArray[0] =
                new NetAddress((u_int8_t*)&addr, sizeof(netAddressBits));
        return;
    }

    // "hostname" is not an IP address string; try resolving it as a real host
    // name instead:
    ...
    host = (struct hostent*)resolvGetHostByName(
            (char*)hostname, (char*)&hostentBuf, sizeof hostentBuf);
#else
...
#else
    // Use "getaddrinfo()" (rather than the older, deprecated
    // "gethostbyname()"):
    struct addrinfo addrinfoHints;
    memset(&addrinfoHints, 0, sizeof addrinfoHints);
    addrinfoHints.ai_family =
            AF_INET;  // For now, we're interested in IPv4 addresses only
    struct addrinfo* addrinfoResultPtr = NULL;
    int result =
            getaddrinfo(hostname, NULL, &addrinfoHints, &addrinfoResultPtr);
...
NetAddressList::~NetAddressList() { clean(); }
...
Port::Port(portNumBits num /* in host byte order */) { fPortNum = htons(num); }
...
AddressPortLookupTable::~AddressPortLookupTable() { delete fTable; }
...
AddressPortLookupTable::Iterator::~Iterator() { delete fIter; }
...
AddressString::AddressString(struct in_addr const& addr) { init(addr.s_addr); }

AddressString::AddressString(netAddressBits addr) { init(addr); }

void AddressString::init(netAddressBits addr) {
    fVal = new char[16];                   // large enough for "abc.def.ghi.jkl"
    netAddressBits addrNBO = htonl(addr);  // make sure we have a value in a
                                           // known byte order: big endian
    sprintf(fVal, "%u.%u.%u.%u", (addrNBO >> 24) & 0xFF, (addrNBO >> 16) & 0xFF,
            (addrNBO >> 8) & 0xFF, addrNBO & 0xFF);
}

AddressString::~AddressString() { delete[] fVal; }
