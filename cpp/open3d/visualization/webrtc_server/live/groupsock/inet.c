 ...
 * inet_addr() as taking no arguments. This confuses C++.  To overcome this, we
 * use our own routine, implemented in C.
 */

unsigned our_inet_addr(cp) char const* cp;
{ return inet_addr(cp); }
...
void our_srandom(unsigned int x) {
    ...
    if (rand_type == TYPE_0)
        ...
        fptr = &state[rand_sep];
        rptr = &state[0];
        for (i = 0; i < 10 * rand_deg; i++) (void)our_random();
    }
}
 ...
 */
char *our_initstate(seed, arg_state, n) unsigned int seed; /* seed for R.N.G. */
 ...
 */
char *our_setstate(arg_state) char *arg_state;
{
...
long our_random() {
    ...
    if (rand_type == TYPE_0) {
        ...
        /* Make copies of "rptr" and "fptr" before working with them, in case
         * we're being called concurrently by multiple threads: */
        long *rp = rptr;
        long *fp = fptr;

        /* Make sure "rp" and "fp" are separated by the correct distance (again,
         * allowing for concurrent access): */
        if (!(fp == rp + SEP_3 || fp + DEG_3 == rp + SEP_3)) {
            /* A rare case that should occur only if we're being called
...
u_int32_t our_random32() {
    /* Return a 32-bit random number.
       Because "our_random()" returns a 31-bit random number, we call it a
       second time, to generate the high bit. (Actually, to increase the
       likelhood of randomness, we take the middle 16 bits of two successive
       calls to "our_random()")
    */
