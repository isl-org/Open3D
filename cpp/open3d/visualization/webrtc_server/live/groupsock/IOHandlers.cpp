...
static unsigned const maxPacketLength =
        50 *
        1024;  // bytes
               // This is usually overkill, because UDP packets are usually no
               // larger than the typical Ethernet MTU (1500 bytes).  However,
               // I've seen reports of Windows Media Servers sending UDP packets
               // as large as 27 kBytes.  These will probably undego lots of
               // IP-level fragmentation, but that occurs below us.  We just
               // have to hope that fragments don't get lost.
