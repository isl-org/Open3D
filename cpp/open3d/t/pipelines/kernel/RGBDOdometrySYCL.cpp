                 ...
                 for (int i = 0; i < 6; ++i) {
                     for (int j = 0; j <= i; ++j) {
                         sycl::atomic_ref<float, sycl::memory_order::acq_rel,
                                 ...
                                 global_sum_ptr[offset++]) += J[i] * J[j];
                     }
                 }
                 for (int i = 0; i < 6; ++i) {
                     sycl::atomic_ref<float, sycl::memory_order::acq_rel,
                                      sycl::memory_scope::device>(
                             global_sum_ptr[21 + i]) += J[i] * d_huber;
                 }
                 sycl::atomic_ref<float, sycl::memory_order::acq_rel,
                                  sycl::memory_scope::device>(
...
void ComputeOdometryResultIntensitySYCL(
    ...
    queue.parallel_for(sycl::range<1>{(size_t)n}, [=](sycl::id<1> id) {
             ...
             if (valid) {
                 const float d_huber = HuberDeriv(r_I, intensity_huber_delta);
                 ...
                 for (int i = 0; i < 6; ++i) {
                     for (int j = 0; j <= i; ++j) {
                         sycl::atomic_ref<float, sycl::memory_order::acq_rel,
                                          sycl::memory_scope::device>(
                                 global_sum_ptr[offset++]) += J_I[i] * J_I[j];
                     }
                 }
                 for (int i = 0; i < 6; ++i) {
                     sycl::atomic_ref<float, sycl::memory_order::acq_rel,
                                      sycl::memory_scope::device>(
                             global_sum_ptr[21 + i]) += J_I[i] * d_huber;
                 }
                 sycl::atomic_ref<float, sycl::memory_order::acq_rel,
                                  sycl::memory_scope::device>(
                         global_sum_ptr[27]) += r_huber;
             ...
             if (valid) {
                 ...
                 for (int i = 0; i < 6; ++i) {
                     sycl::atomic_ref<float, sycl::memory_order::acq_rel,
                                      sycl::memory_scope::device>(
                             global_sum_ptr[21 + i]) +=
                             J_I[i] * HuberDeriv(r_I, intensity_huber_delta) +
             ...
             if (valid) {
                 ...
                 for (int i = 0; i < 6; ++i) {
                     for (int j = 0; j <= i; ++j) {
                         sycl::atomic_ref<float, sycl::memory_order::acq_rel,
                                          sycl::memory_scope::device>(
                                 global_sum_ptr[offset++]) += J_x[i] * J_x[j] +
                                                              J_y[i] * J_y[j] +
                                                              J_z[i] * J_z[j];
                     }
                 }
