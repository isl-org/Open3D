...
TriangleMesh TriangleMesh::InterpolateTextureCoordinatesFrom(
    ...
    struct Vector2dEqual {
        bool operator()(const Eigen::Vector2d &v1,
                        const Eigen::Vector2d &v2) const {
            return v1.isApprox(v2);
        }
    };
    using vector2d_set =
            std::unordered_set<Eigen::Vector2d,
                               utility::hash_eigen<Eigen::Vector2d>,
                               Vector2dEqual>;
    ...
    for (size_t i = 0; i < input_mesh.triangles_.size(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            ...
            if (vertex_to_uvs[vertex_idx].find(uv) ==
                vertex_to_uvs[vertex_idx].end()) {
    ...
    auto get_best_uvs = [&distance](const vector2d_set &uvs1,
        ...
        std::vector<Eigen::Vector2d> best_uvs{Eigen::Vector2d(0, 0),
                                              Eigen::Vector2d(0, 0),
                                              Eigen::Vector2d(0, 0)};
        if (uvs1.size() == 0 || uvs2.size() == 0 || uvs3.size() == 0)
            return best_uvs;
        ...
        for (auto &uv1 : uvs1) {
            for (auto &uv2 : uvs2) {
                for (auto &uv3 : uvs3) {
                    auto aggregate_distance = distance(uv1, uv2) +
                                              distance(uv2, uv3) +
                                              distance(uv3, uv1);
    ...
    for (const auto &triangle : triangles_) {
        auto best_uvs = get_best_uvs(vertex_to_uvs[triangle(0)],
                                     vertex_to_uvs[triangle(1)],
                                     vertex_to_uvs[triangle(2)]);
