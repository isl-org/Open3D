...
cpp_test() {
    ...
    if [ "${BUILD_SYCL_MODULE}" == "ON" ]; then
        ...
        # Dataset tests default to $HOME/open3d_data; give each shard its own HOME
        # so parallel downloads and extracts do not corrupt shared files.
        ${docker_run} -i --rm "${DOCKER_TAG}" /bin/bash -euo pipefail -c " \
            cd build \
         && seq 0 $((${gtest_shards} - 1)) | parallel -k --jobs ${gtest_shards} --halt never \
            'd=/tmp/open3d-gtest-shard-{}; mkdir -p "\$d/home/open3d_data" "\$d/tmp" && HOME="\$d/home" TMPDIR="\$d/tmp" OPEN3D_DATA_ROOT="\$d/home/open3d_data" GTEST_TOTAL_SHARDS='"${gtest_shards}"' GTEST_SHARD_INDEX={} ./bin/tests --gtest_shuffle' \
        "
    else
