## Main
-   Implement `Transform` for the legacy `OrientedBoundingBox` and `OrientedBoundingEllipsoid` for similarity transforms (rotation, uniform scale, translation) instead of always throwing. Shear, non-uniform scale, mirroring and projective transforms still raise, with a message naming the offending part (PR #7559) (issue #7558)
