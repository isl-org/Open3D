...
-   Add `NearestToCentroid` and `NearestToCenter` reduction modes to `PointCloud::VoxelDownSample` via a new `VoxelReduction` enum. Both new modes return one of the original input points (non-deforming spatial subsampling) with its attributes copied through unchanged; the default `Centroid` mode preserves the existing averaged behaviour. Matches PDAL's `VoxelCentroidNearestNeighbor` and `VoxelCenterNearestNeighbor` filters (issue #6807).
