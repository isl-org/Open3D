...
# of horizontal layouts for each row.
...
if gui.Application.instance.menubar is None:
    ...
    # always be the name of the application (probably "Python"), regardless
    # of what you pass in here. The application menu is typically where
    # About..., Preferences..., and Quit go.
...
# Create a file-chooser widget. One part will be a text edit widget for the
# filename and clicking on the button will let the user choose using the
# file dialog.
...
layout.add_child(fileedit_layout)  # add to the top-level (vertical) layout
...
# use. All layouts take a spacing parameter, which is the spacinging between
...
# a binary property, but in this case it will show a simple message box, which
...
# Create a color editor. We will change the color of the orange label above
...
# This is a combobox, nothing fancy here, just set a simple function to handle
...
# Number editor can clamp numbers to a range, although this is more useful
# for integers than for floating point.
...
numlayout.add_fixed(em)  # manual spacing (could have set it in Horiz() ctor
...
# Create a slider. It acts very similar to NumberEdit except that the user
# moves a slider and cannot type the number.
...
# Create a text editor. The placeholder text (if not empty) will be displayed
# when there is no text, as concise help, or visible tooltip.
tedit = gui.TextEdit()
...
# on_text_changed fires whenever the user changes the text (but not if the
# text_value property is assigned to).
tedit.set_on_text_changed(on_text_changed)
# on_value_changed fires whenever the user signals that they are finished
# editing the text, either by pressing return or by clicking outside of the
# text editor, thus losing text focus.
...
# Create a VGrid layout. This layout specifies the number of columns (two,
# in this case), and will place the first child in the first column, the
# second in the second, the third in the first, the fourth in the second, etc.
# So:
...
# Create a tab control. This is really a set of N layouts on top of each other,
# but with only one selected.
tabs = gui.TabControl()
tab1 = gui.Vert()
...
w.add_child(layout)  # set the window's layout
gui.Application.instance.add_window(w)  # make the window visible
