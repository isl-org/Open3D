...
def load_fragment_rgbd(config):
    color_list_fnames = sorted(
        glob.glob(os.path.join(config.path_dataset, "fragments", "*_colors.txt"))
    )
    depth_list_fnames = sorted(
        glob.glob(os.path.join(config.path_dataset, "fragments", "*_depths.txt"))
    )
...
def load_fragment_pcd(config):
    pcd_list_fnames = sorted(
        glob.glob(os.path.join(config.path_dataset, "fragments", "fragment_pcd_*.ply"))
    )

    return pcd_list_fnames


def load_fragment_posegraph(config):
    pose_graph_fnames = sorted(
        glob.glob(
            os.path.join(config.path_dataset, "fragments", "fragment_posegraph_*.json")
        )
    )

    return pose_graph_fnames
