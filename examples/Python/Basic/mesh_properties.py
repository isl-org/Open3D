...
def check_properties(name, mesh):
    ...
    if not edge_manifold:
        ...
        o3d.visualization.draw_geometries([mesh, edges_to_lineset(mesh, edges, (1,0,0))])
    if not edge_manifold_boundary:
        edges = mesh.get_non_manifold_edges(allow_boundary_edges=False)
        print('  # visualize non-manifold edges (allow_boundary_edges=False)')
        o3d.visualization.draw_geometries([mesh, edges_to_lineset(mesh, edges, (0,1,0))])
    if not vertex_manifold:
        verts = np.asarray(mesh.get_non_manifold_vertices())
        print('  # visualize non-manifold vertices')
        pcl = o3d.geometry.PointCloud()
        pcl.points = o3d.utility.Vector3dVector(np.asarray(mesh.vertices)[verts])
    ...
    if self_intersecting:
        ...
        o3d.visualization.draw_geometries([mesh, edges_to_lineset(mesh, edges, (1,1,0))])
