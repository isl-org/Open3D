...
docker container exec -it -w $Open3D_DOCK $NAME bash -c 'mkdir -p build && cd build && cmake .. -DCMAKE_INSTALL_PREFIX=~/open3d_install  -DBUILD_UNIT_TESTS=ON && make -j && make install && bash'
