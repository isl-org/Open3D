...
from ...classes import DTYPE_MAP
import paddle

__all__ = ['FixedRadiusSearch', 'RadiusSearch', 'KNNSearch']


class FixedRadiusSearch(paddle.nn.Layer):
    ...
    def __init__(self,
        ...
        assert index_dtype in [paddle.int32, paddle.int64]
        self.index_dtype = DTYPE_MAP[index_dtype]
    ...
    def forward(self,
        ...
        if points_row_splits is None:
            points_row_splits = paddle.to_tensor([0, points.shape[0]],
                                                 dtype="int64")
        if queries_row_splits is None:
            queries_row_splits = paddle.to_tensor([0, queries.shape[0]],
                                                  dtype="int64")
...
class RadiusSearch(paddle.nn.Layer):
...
class KNNSearch(paddle.nn.Layer):
