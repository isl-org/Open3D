...
- ``-DUSE_PREINSTALLED_GLEW=OFF -DUSE_PREINSTALLED_GLFW=OFF``: note that headless rendering only works with the **glew 2.1** and **glfw 3.3-dev** version.
...
.. code-block:: shell
    ...
    (py3env) $ cmake -DENABLE_HEADLESS_RENDERING=ON \
                     -DENABLE_GUI=OFF \
                     -DUSE_PREINSTALLED_GLEW=OFF \
                     -DUSE_PREINSTALLED_GLFW=OFF \
                     ..
...
Try ``cmake`` with ``-DUSE_PREINSTALLED_GLEW=OFF`` and ``-DUSE_PREINSTALLED_GLFW=OFF`` flags.
...
.. code-block:: shell
    ...
    (py3env) $ cd ~/Open3D
    (py3env) $ mkdir build&&cd build
    (py3env) $ cmake -DENABLE_HEADLESS_RENDERING=ON -DUSE_PREINSTALLED_GLEW=OFF -DUSE_PREINSTALLED_GLFW=OFF \
