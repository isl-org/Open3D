...
``ScalableTSDFVolume`` has several parameters. ``voxel_length = 4.0 / 512.0`` means a single voxel size is  :math:`\frac{4.0m}{512.0} = 7.8125mm`. ``sdf_trunc = 0.04`` specifies truncation value for signed distance function (SDF). When ``with_color = True``, color is also integrated as part of the TSDF volume. The color integration is inspired by `PCL <http://pointclouds.org/>`_.
