...
Dense RGB-D SLAM
-------------------------------------
Equipped with the fast volumetric reconstruction backend, we in addition provide a dense RGB-D SLAM system using frame-to-model tracking. The tensor implementation supports CPU, CUDA, and SYCL devices. The example can be found at ``examples/python/t_reconstruction_system/dense_slam.py`` for the command line version and ``examples/python/t_reconstruction_system/dense_slam_gui.py`` for a GUI demo. Similar C++ versions can be found at ``examples/cpp/OfflineSLAM.cpp`` and ``examples/cpp/OnlineSLAMRGBD.cpp``.

For an Intel GPU, build Open3D with SYCL enabled and pass ``--device SYCL:0``.
The tensor RGB-D odometry methods ``point2plane``, ``intensity``, and ``hybrid``
are supported by the SYCL reconstruction path.
